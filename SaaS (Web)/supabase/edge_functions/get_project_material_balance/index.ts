// Edge Function: get_project_material_balance
// Returns net material balance by project/material for the authenticated tenant.
...
serve(async (req) => {
  ...
  if (appUserError || !appUser?.tenant_id || appUser.ativo === false) {
    return respond(403, { success: false, message: 'Usuario sem permissao para consultar saldo por projeto.' })
  }
  ...
  let query = supabase
    ...
    .select('projeto, material_id, qty_issued, qty_returned, qty_net, updated_at, materials!inner(codigo, descricao, umb, tipo)', { count: 'exact' })
  ...
  if (qtyExact !== null) query = query.eq('qty_net', qtyExact)
  if (qtyMin !== null) query = query.gte('qty_net', qtyMin)
  if (qtyMax !== null) query = query.lte('qty_net', qtyMax)

  const { data, error, count } = await query
  if (error) return respond(500, { success: false, message: 'Falha ao consultar saldo de materiais por projeto.' })
