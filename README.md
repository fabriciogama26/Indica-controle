...
- `api/teams/meta/route.ts`: carrega bases, tipos, encarregados e supervisores validos para a tela de Equipes.
  ...
  - `api/materials/route.ts`: cadastra, edita, cancela/ativa, lista e consulta historico de materiais por tenant, com filtro textual por `UMB`, validacao de `Tipo` (`NOVO`/`SUCATA`), suporte a rastreio por serial e `Preco` opcional (default `0.00`).
