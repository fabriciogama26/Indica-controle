...
- `src/app/`
  ...
  - `(dashboard)/dashboard-medicao/page.tsx`: rota do Dashboard Medicao com filtros de ciclo, projeto digitavel por SOB e status de execucao, mantendo os blocos `Concluidos X parciais no ciclo`, `Visao geral por periodo` e `Ciclo da medicao`.
  - `(dashboard)/dashboard-equipes/page.tsx`: rota read-only do Dashboard Equipes com filtros de ciclo, data inicial/final, projeto, MK/equipe, encarregado e supervisor; metas, ranking, bullet e gap oficiais por `team_id`; encarregados separados por `team_id + foreman_name_snapshot`; modal do MK com valor, meta integral, contribuicoes, dias, ordens, projetos e exportacao CSV.
  - `(dashboard)/dashboard-carteira-operacional/page.tsx`: rota read-only do Dashboard Carteira Operacional, baseado em projetos com atividades previstas cadastradas, calculando valor previsto por `Pontos x Quantidade prevista x Valor unitario x taxa(1)`, com filtro de carteira `Sem retirados`/`Somente retirados`/`Com retirados`, todos os ciclos, saude, financeiro, cobertura da meta por RPC, renovacao, envelhecimento e tabela analitica da carteira com exportacao CSV e acao para ver atividades previstas por projeto.
  ...
  - `api/projects/activity-forecast/route.ts`: lista, adiciona e edita atividades previstas por projeto com controle de concorrencia por `updated_at` na edicao, devolvendo pontos e valor previsto por linha.
...
- `api/stock-balance/route.ts`: lista o saldo atual por centro/material com filtros e paginacao real no banco (`count:"exact"` + `.range()`); por padrao usa caminho rapido sem hidratar materiais historicos zerados; recomposicao de saldo `0` via `stock_transfers` ativada apenas quando `includeHistoricalZeros=1` (opt-in); historico por material/centro via `?mode=history` agora busca items do material primeiro (limit 2000) e cruza com transfers do centro, evitando varredura irrestrita; historico enriquecido com `Equipe`/`Encarregado` e filtro por operacao/origem.
  ...
  - `api/dashboard-medicao/route.ts`: autentica e exige `dashboard-medicao/read`, consulta fontes pelo tenant, aplica filtros, calcula status, garantia minima, tickets, ritmo e projecao e delega as consolidacoes de equipes, encarregados e supervisores ao modulo server-side `team-performance`.
  - `api/dashboard-equipes/route.ts`: exige `dashboard-equipes/read` e reutiliza o controller server-side de leitura para entregar o recorte operacional da nova tela.
...
- `src/modules/dashboard/dashboard-medicao/`
  - `DashboardMeasurementPageView.tsx`: dashboard geral da Medicao com filtros de ciclo, projeto, status e ano do grafico anual, mantendo `Concluidos X parciais no ciclo`, `Visao geral por periodo`, `Evolucao anual por ciclo` e `Ciclo da medicao`.
  - `DashboardMeasurementPageView.module.css`: estilos do Dashboard Medicao.
- `src/modules/dashboard/dashboard-equipes/`
  - `types.ts`: contratos do endpoint e das linhas de equipe, encarregado e supervisor.
  - `constants.ts`: endpoint e estado inicial dos filtros.
  - `utils.ts`: formatacao de moeda e percentual.
  - `api.ts`: cliente autenticado de `/api/dashboard-equipes`.
  - `hooks.ts`: carregamento, filtros e log de erros com tag `dashboard_equipes`.
  - `DashboardTeamsPageView.tsx`: tela inicial read-only por MK/equipe.
  - `DashboardTeamsPageView.module.css`: estilos responsivos da tela.
  - `index.ts`: fachada publica do modulo.
- `src/modules/dashboard/dashboard-carteira-operacional/`
  - `DashboardPortfolioPageView.tsx`: tela read-only da carteira operacional com filtros, diagnostico, cards, graficos simples e tabela paginada.
  - `DashboardPortfolioPageView.module.css`: estilos dedicados da tela.
  - `types.ts`: contratos do payload de carteira.
  - `constants.ts`: endpoint, filtros vazios e tamanho de pagina da tabela.
  - `utils.ts`: formatadores de moeda, percentual, numero, status e origem.
  - `api.ts`: cliente autenticado de `/api/dashboard-carteira-operacional`.
  - `hooks.ts`: carregamento, estado dos filtros, payload e erro da tela.
  - `index.ts`: fachada publica do modulo.
- `src/modules/dashboard/dash-estoque/`
  - `StockDashboardPageView.tsx`: Dashboard Estoque com filtros, bloco de estoque por UMB, graficos, dispersao com cards por UMB conforme Requisicao/Devolucao, exportacao Excel (CSV) da tabela, legenda acima da area plotada, lista de materiais com UMB e evolucao por operacoes distintas.
...
- `src/modules/dashboard/dash-operacional-faturamento/`
  - `OperationalBillingDashboardPageView.tsx`: dashboard comparativo por projeto entre Medicao, Medicao Asbuilt e Faturamento, com filtros obrigatorios por Centro de servico e Projeto, filtro por codigo, status ativa/inativa, divergencias, ausencias, resumo, grafico operacional com filtro proprio, select `Servicos considerados ate` tambem no bloco de indicadores e no modal operacional, filtro de Taxa no modal operacional, barra segmentada por corte no modo `Todos`, modal de detalhamento das faixas de Asbuilt por projeto, modal com abas `Medicao`, `M. As built`, `As Built` e `Faturado` por card operacional, cards de Ticket medio / Projetos e Ticket medio / Servicos para Medicao e Asbuilt, cards globais por categoria instalada/retirada e categorias `PODA`/`ARRASTO` comparando quantidade e valor por origem, tabela clicavel de valor/projetos/medicoes do grafico, tabela consolidada por codigo e tabela de categorias cobradas.
  - `OperationalBillingDashboardPageView.module.css`: estilos do Dash operacional e faturamento, incluindo filtros, barras do grafico operacional, cards de tickets medios, cards globais medidos com quatro origens por card, tabela complementar clicavel do grafico e modais de detalhamento.
...
15. O shell principal libera navegacao para as secoes `Visao Geral`, `Operacao`, `Almoxarifado`, `Cadastros` e `Cadastro Base`; `Dashboard Medicao`, `Dashboard Equipes`, `Dashboard Carteira Operacional` e `Dash operacional e faturamento` ficam em `Visao Geral`, e `Meta` e `Atividades` ficam agrupadas em `Cadastros`.
  - Em `Configuracao do Mapa do Almoxarifado`, o usuario define grid, prateleiras, andares e posicoes por centro de estoque; em `Mapa do Almoxarifado`, consulta ocupacao por material e atribui/remove endereco fisico sem movimentar saldo.
...
18. A aba `Atividades previstas` em `/projetos` permite selecionar projeto, pesquisar atividades ativas, incluir manualmente, editar quantidade/observacao na linha, visualizar pontos e valor previsto (`Pontos x Quantidade prevista x Valor unitario x taxa(1)`), baixar modelo XLSX via Edge Function `get_project_activity_forecast_template`, importar previsao via Edge Function `import_project_activity_forecast` (colunas `codigo` e `quantidade`) e exportar CSV via `/api/projects/activity-forecast`.
...
35. A rota `/medicao` opera `Ordem de Medicao` com persistencia transacional via `/api/medicao`, lista paginada no servidor, filtros por `Tipo de Servico` do projeto, `Atividade`, `Tipo` e `Motivo sem producao`, modos `Com producao` e `Sem producao`, motivo estruturado por tenant para ordens sem producao, cabecalho pre-preenchido quando aberta pela acao `Fazer medicao` da Composicao de Equipe, taxa unica por ordem reaplicada a todos os itens na edicao, sugestao automatica de taxa por projeto (ultima medicao) inclusive em ordens `Sem producao`, match automatico opcional por `Projeto principal legado + Equipe + Data`, indicador que informa na lista e nos detalhes se existe composicao ativa da equipe na data de execucao, cards filtrados de Garantia de faturamento minimo e valor total descontando essa garantia, historico, exportacoes e cadastro em massa CSV com suporte aos dois tipos de ordem.
...
37. A rota `/dashboard-medicao` exibe os indicadores gerais da Medicao por ciclo operacional 21 a 20: `Concluidos X parciais no ciclo`, `Visao geral por periodo`, `Evolucao anual por ciclo` em grafico combinado com Medicao em barras, Meta ciclo em linha tracejada e Previsao em linha para os ciclos com dados do ano selecionado, e `Ciclo da medicao`. Os blocos operacionais por equipe, encarregado e supervisor ficam em `/dashboard-equipes`, e a analise de causa da carteira fica em `/dashboard-carteira-operacional`, com filtros e permissoes independentes.
38. A rota `/dashboard-carteira-operacional` analisa projetos com atividades previstas cadastradas em Projetos, exclui teste/terceiros/inativos, calcula o valor previsto via RPC por `Pontos x Quantidade prevista x Valor unitario x taxa(1)`, permite filtrar `Sem retirados`, `Somente retirados`, `Com retirados` e todos os ciclos, trata estados parciais como pendentes, separando saude por quantidade, financeiro da carteira, cobertura da meta por `dashboard_portfolio_goal_coverage`, fluxo, renovacao, envelhecimento, tooltips de parametros nos blocos analiticos, diagnostico automatico e tabela analitica com dias parado/ciclos sem producao, exportacao CSV e acao para ver atividades previstas.
39. A rota `/atividades` permite cadastrar, editar, consultar detalhes/historico e cancelar/ativar atividades no tenant atual, exigindo `codigo`, `descricao`, `tipo`, `categoria`, `grupo`, `valor`, `pontos` e `unidade`, com `alcance` opcional em texto livre, categorias em ordem alfabetica no cadastro/filtro, uso de `/api/activities` com listagem paginada no servidor e escrita delegada para as RPCs `save_service_activity_record` e `set_service_activity_record_status`.
40. A rota `/programacao` passa a usar `project_programming_history` como timeline operacional dedicada da agenda, com `REPROGRAMADA` salvo fisicamente em `project_programming` e historico de `CREATE/UPDATE/RESCHEDULE/BATCH_CREATE` gravado dentro das RPCs transacionais full, sem depender de `app_entity_history` nem de historico complementar pos-commit na API.
41. A rota `/pessoas` permite cadastrar manualmente ou em massa por CSV, editar, consultar detalhes/historico e cancelar/ativar pessoas no tenant atual, usando `/api/people` com escrita delegada para as RPCs `save_person_record` e `set_person_record_status`, mantendo `Matricula` unica por tenant, `CPF` unico quando preenchido, `Telefone` opcional e `Tipo` obrigatorio somente para cargos operacionais especificos.
42. A rota `/permissoes` continua enviando convite pelo backend, mas a auditoria do invite passa a ser gravada pela RPC `append_user_invite_history`.
43. A migration `050_activity_code_precheck_and_optional_fields.sql` torna `grupo/alcance` opcionais em `service_activities` e adiciona o RPC `precheck_activity_code_conflict` para bloquear codigo duplicado por tenant.
44. A migration `051_create_app_entity_history_and_activity_status.sql` cria `app_entity_history` (historico generico reutilizavel por outras telas) e adiciona em `service_activities` os campos de cancelamento/ativacao com motivo e data.
45. A migration `180_require_service_activity_group_in_rpc.sql` recompila `save_service_activity_record` para exigir `group_name` no cadastro/edicao de Atividades; a migration `181_create_serial_retirement_flow.sql` cria o fluxo `RET` de unidade por serial, com auditoria em `serial_retirements`, baixa de saldo disponivel, manutencao da presenca fisica em `trafo_instances` e movimentacao fisica posterior sem recompor disponibilidade.
46. A migration `042_materials_price_status_and_history.sql` adiciona `unit_price`, status ativo/inativo e historicos de materiais, alem de remover `lp` e `serial` do cadastro base.
...
71. A rota `/composicao-equipe` permite adicionar mais de um projeto na mesma composicao; os projetos ficam em `team_composition_projects`, enquanto `team_compositions.project_id` preserva o primeiro projeto como referencia legada.
72. A lista de `/composicao-equipe` possui a acao `Fazer medicao`, que abre `/medicao` com Projeto, Equipe e Data preenchidos a partir da composicao quando existe projeto vinculado.
...
- `Estado Trabalho ANTECIPADO nao esta ativo no catalogo do tenant atual`:
  - Causa: o usuario salvou uma etapa como `CONCLUIDO`, mas o catalogo do tenant ainda nao possui o codigo `ANTECIPADO` exigido para atualizar etapas posteriores.
  - Solucao: aplicar `255_add_anticipated_work_completion_status.sql` e repetir o salvamento.
