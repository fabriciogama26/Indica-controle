...
- `src/app/`
  ...
  - `(dashboard)/dashboard-medicao/page.tsx`: rota do Dashboard Medicao com filtros de ciclo, projeto digitavel por SOB, equipe, encarregado, supervisor e status execucao com Pendencias e Parcial planejado beneficio atingido aplicados apenas pelo botao `Filtrar`, grafico operacional do ciclo e `Visao geral por periodo` com Garantia de faturamento minimo lado a lado em desktop, cards acima da tabela, tabela com linha de total, coluna de projetos e cards de Ticket medio / Projetos, Ticket medio / Servicos, Projetos no ciclo e Ordens de Servicos no ciclo, periodo `De/Para` local iniciado no ano calendario corrente e com botao proprio de filtro, comparativo contra as tres metas com cards de Ritmo atual, Ritmo produtivo por equipes/dias com producao e Ritmo meta pela quantidade de equipes definida no ciclo, alem das duas diferencas na tabela, cards de Ticket medio / Projetos, Ticket medio / Servicos, Projetos no ciclo e Ordens de Servicos no ciclo, coluna de projetos e barra de Projecao de fechamento, tabela oficial `Equipes no ciclo`, analise gerencial `Encarregados no ciclo`, `Supervisor no ciclo` com meta por equipes que produziram ou por todas as equipes vinculadas e coluna de projetos, filtros locais por semana, modal de projetos por categoria/equipe/encarregado/supervisor com valor cobrado, centro e exportacao Excel CSV, quadro grafico com `% atingimento`, `Bullet de meta` e `Gap financeiro`, legendas por grafico acima do texto do ciclo/semana, ranking de atingimento, bullet chart, gap financeiro, expansao individual dos graficos de encarregados, checkboxes de metas, dias dinamicos por meta, legenda e modal de grafico ampliado.
  - `(dashboard)/dash-estoque/page.tsx`: rota do Dashboard Estoque com indicadores de saldo, giro, curva ABC, dispersao de materiais e evolucao mensal pela quantidade de operacoes realizadas.
  - `(dashboard)/dash-operacional-faturamento/page.tsx`: rota do Dash operacional e faturamento com comparativo por projeto entre Medicao, Medicao Asbuilt e Faturamento, filtros obrigatorios por Centro de servico e Projeto, filtro por codigo de atividade, status ativa/inativa, divergencias, ausencias, grafico operacional com filtro proprio, tickets medios por projetos e servicos para Medicao e Asbuilt, doze cards globais por categoria instalada/retirada comparando quantidades de Medidos, ASBUILT e Faturado, tabela de valor/projetos/medicoes do grafico, tabela de categorias cobradas e exportacao CSV.
  ...
  - `(dashboard)/materiais/page.tsx`: rota da tela de Materiais com cadastro, filtros por Codigo, Descricao, UMB, Tipo e Status, e listagem, incluindo `Tipo` por select (`NOVO`/`SUCATA`), rastreio por serial e `Preco` opcional.
  ...
  - `api/meta/route.ts`: carrega e salva em cadastro unico as metas de medicao por tipo de equipe e dias uteis por ciclo, exige permissao server-side `meta/read`, `meta/create` ou `meta/update`, recalcula a media atual de dias trabalhados nos cards, lista e detalhe sem gravar durante a leitura, filtra pelo tenant autenticado, forma ciclos a partir das datas reais de medicao e expoe historico das metas salvas.
...
- `api/stock-balance/route.ts`: lista o saldo atual por centro/material com filtros, paginacao server-side, exclui centros de equipe da tela de Estoque Atual, recompõe materiais historicos com saldo `0` nos centros fisicos quando necessario e mantem historico enriquecido com `Equipe`/`Encarregado`, incluindo filtro por operacao/origem para localizar `Retorno de campo` via `CAMPO / INSTALADO`.
  ...
  - `api/dashboard-medicao/route.ts`: consolida indicadores de Medicao por ciclo, status economico, Pendencias, Parcial planejado beneficio atingido, meta, encarregado e supervisor, exige permissao server-side `dashboard-medicao/read`, monta semanas sequenciais do ciclo 21 a 20, aplica filtro de projeto por SOB, calcula Concluidos x Parciais x Parcial planejado beneficio atingido x Pendencias do ciclo e da `Visao geral por periodo`, inclui Garantia de faturamento minimo no recorte `De/Para` a partir de ordens Sem producao com valor positivo no cabecalho, calcula projetos distintos, ticket medio por projetos, ticket medio por servicos e lista de projetos por categoria/equipe/encarregado/supervisor com valor cobrado e centro, calcula dias trabalhados e a media dinamica de dias reais por equipes filtradas com producao, consolida a meta oficial por `team_id`, o Ritmo produtivo por equipe + data e o Ritmo meta pela soma da meta diaria das equipes definidas no ciclo, rateia as tres metas gerenciais somente entre encarregados salvos nas medicoes validas conforme seus dias medidos por equipe, soma producao por supervisor vinculado as equipes, calcula meta de supervisor por equipes que produziram e por todas as equipes ativas vinculadas, retorna consolidacoes por ciclo e por semana, calcula a projecao de fechamento do ciclo e filtra tudo pelo tenant autenticado.
  - `api/dash-estoque/route.ts`: consolida saldos e movimentacoes do Dashboard Estoque por tenant, contando operacoes distintas por transferencia e tipo, sem confundir quantidade de itens com quantidade de operacoes.
  - `api/dash-operacional-faturamento/route.ts`: consolida codigos de atividade usados em Medicao, Medicao Asbuilt e Faturamento por projeto, respeitando Centro de servico, status de atividade, divergencias e ausencias, agrega categorias cobradas por `service_activities.type_service`, soma cards globais paginados de Medidos, ASBUILT e Faturado para postes, rede, equipamentos, transformadores, cruzetas e estruturas instalados/retirados, calcula tickets medios por projetos e servicos para Medicao e Asbuilt e monta o grafico operacional de Total medido, Medido (AS BUILT), As Built e Faturado com valor, projetos e medicoes, sempre filtrado pelo tenant autenticado.
...
- `src/modules/dashboard/dashboard-medicao/`
  - `DashboardMeasurementPageView.tsx`: dashboard da Medicao com filtros gerais aplicados apenas pelo botao `Filtrar`, filtro Supervisor, filtro Status execucao com Pendencias e Parcial planejado beneficio atingido, tabelas de status com linha de total, grafico operacional do ciclo e `Visao geral por periodo` com Garantia de faturamento minimo lado a lado em desktop com cards acima da tabela, coluna de projetos e cards de Ticket medio / Projetos, Ticket medio / Servicos, Projetos no ciclo e Ordens de Servicos no ciclo, periodo `De/Para` iniciado no ano calendario corrente, botao proprio para aplicar periodo, seletor de base de meta do ciclo, cards de Ticket medio / Projetos, Ticket medio / Servicos, Projetos no ciclo e Ordens de Servicos no ciclo, coluna de projetos, barra de Projecao de fechamento entre valor realizado e meta, ritmo atual, diferenca prevista, modal de projetos por categoria/equipe/encarregado/supervisor com valor cobrado, centro e exportacao Excel CSV, tabela oficial `Equipes no ciclo`, quadro gerencial `Encarregados no ciclo`, `Supervisor no ciclo` com seletor de meta por equipes que produziram ou por todas as equipes vinculadas, selects locais de semana nos blocos de equipes/encarregados/supervisor e quadro grafico com `% atingimento`, `Bullet de meta` e `Gap financeiro`, legendas por grafico acima do texto do ciclo/semana, tabela unica de encarregados com coluna de projetos, ranking de atingimento com referencia 100%, bullet chart de metas, gap financeiro, botoes individuais de expansao, checkboxes de metas, dias dinamicos por meta e modal ampliado.
  - `DashboardMeasurementPageView.module.css`: estilos do Dashboard Medicao.
- `src/modules/dashboard/dash-estoque/`
  - `StockDashboardPageView.tsx`: Dashboard Estoque com filtros, graficos, dispersao com legenda acima da area plotada, lista de materiais com UMB e evolucao por operacoes distintas.
  - `StockDashboardPageView.module.css`: estilos dos indicadores, graficos, tabela e modal do Dashboard Estoque.
- `src/modules/dashboard/dash-operacional-faturamento/`
  - `OperationalBillingDashboardPageView.tsx`: dashboard comparativo por projeto entre Medicao, Medicao Asbuilt e Faturamento, com filtros obrigatorios por Centro de servico e Projeto, filtro por codigo, status ativa/inativa, divergencias, ausencias, resumo, grafico operacional com filtro proprio, cards de Ticket medio / Projetos e Ticket medio / Servicos para Medicao e Asbuilt, doze cards globais por categoria instalada/retirada comparando quantidades de Medidos, ASBUILT e Faturado, tabela de valor/projetos/medicoes do grafico, tabela consolidada por codigo e tabela de categorias cobradas.
  - `OperationalBillingDashboardPageView.module.css`: estilos do Dash operacional e faturamento, incluindo filtros, barras do grafico operacional, cards de tickets medios, cards globais medidos organizados em duas linhas de seis cards no desktop e tabela complementar do grafico.
...
15. O shell principal libera navegacao para as secoes `Visao Geral`, `Operacao`, `Almoxarifado`, `Cadastros` e `Cadastro Base`; `Dashboard Medicao` e `Dash operacional e faturamento` ficam em `Visao Geral`, e `Meta` e `Atividades` ficam agrupadas em `Cadastros`.
...
34. A rota `/materiais` permite cadastrar, editar, cancelar/ativar e filtrar materiais por Codigo, Descricao, UMB, Tipo e Status no tenant atual usando a rota `/api/materials`, com `Tipo` por select (`NOVO`/`SUCATA`), rastreio por serial e `Preco` opcional; a persistencia e o historico seguem delegados para as RPCs `save_material_record` e `set_material_record_status`.
35. A rota `/medicao` opera `Ordem de Medicao` com persistencia transacional via `/api/medicao`, lista paginada no servidor, filtros por `Tipo de Servico` do projeto, `Atividade`, `Tipo` e `Motivo sem producao`, modos `Com producao` e `Sem producao`, motivo estruturado por tenant para ordens sem producao, taxa unica por ordem reaplicada a todos os itens na edicao, sugestao automatica de taxa por projeto (ultima medicao), match automatico opcional por `Projeto + Equipe + Data`, indicador que informa na lista e nos detalhes se existe composicao ativa da equipe na data de execucao, cards filtrados de Garantia de faturamento minimo e valor total descontando essa garantia, historico, exportacoes e cadastro em massa CSV com suporte aos dois tipos de ordem.
...
37. A rota `/dashboard-medicao` exibe indicadores da Medicao por ciclo operacional 21 a 20, compara valor realizado com a Meta, separa Concluidos x Parciais x Parcial planejado beneficio atingido x Pendencias no grafico operacional do ciclo e exibe `Visao geral por periodo` `De/Para` independente do ciclo selecionado com as mesmas categorias mais Garantia de faturamento minimo, lado a lado em desktop, mostra cards acima da tabela no periodo, linha de total nas tabelas de status, coluna de projetos, inicia o periodo no ano calendario corrente, aplica filtros gerais somente pelo botao `Filtrar`, aplica o periodo somente pelo botao `Filtrar periodo`, calcula dias trabalhados por datas distintas de `Data execucao` com medicao `Com producao` no ciclo, mostra cards de Ticket medio / Projetos, Ticket medio / Servicos, Projetos no ciclo, Ordens de Servicos no ciclo, Ritmo atual, Ritmo produtivo e Ritmo meta, calcula o produtivo somente por equipe + data com medicao `COM_PRODUCAO` ativa e nao cancelada, calcula o ritmo meta pela quantidade de equipes definida na Meta do ciclo, mostra na tabela as diferencas do ritmo atual contra as duas referencias, exibe `Projecao de fechamento` entre Valor realizado e Meta, com dias da meta selecionada e diferenca prevista contra a meta selecionada, ocultando a projecao quando a selecao for `Meta ciclo trabalhado`, filtra Projeto por SOB digitavel, filtra Supervisor pelo vinculo da equipe, filtra Status execucao tambem por Pendencias e Parcial planejado beneficio atingido, mostra a tabela oficial `Equipes no ciclo` por `team_id`, tipos vigentes e encarregados historicos, mantém `Encarregados no ciclo` como analise gerencial somente para encarregados salvos nas medicoes validas e rateia as tres metas conforme os dias medidos por equipe, mostra `Supervisor no ciclo` com alternancia entre meta por equipes que produziram e meta por todas as equipes vinculadas, filtros locais por semana, coluna de projetos nas tabelas, modal de projetos por categoria/equipe/encarregado/supervisor com valor cobrado, centro, ordens, totalizador e exportacao Excel CSV, quadro grafico com `% atingimento`, `Bullet de meta` e `Gap financeiro`, legendas por grafico acima do texto do ciclo/semana, ranking % com linha 100% centralizada, bullet chart de metas, gap financeiro, expansao individual dos graficos de encarregados, legenda e checkboxes de metas, ajusta a coluna Dias conforme a meta e amplia graficos em modal.
38. A rota `/atividades` permite cadastrar, editar, consultar detalhes/historico e cancelar/ativar atividades no tenant atual, exigindo `codigo`, `descricao`, `tipo`, `categoria`, `grupo`, `valor`, `pontos` e `unidade`, com `alcance` opcional em texto livre, categorias em ordem alfabetica no cadastro/filtro, uso de `/api/activities` com listagem paginada no servidor e escrita delegada para as RPCs `save_service_activity_record` e `set_service_activity_record_status`.
...
40. A rota `/pessoas` permite cadastrar manualmente ou em massa por CSV, editar, consultar detalhes/historico e cancelar/ativar pessoas no tenant atual, usando `/api/people` com escrita delegada para as RPCs `save_person_record` e `set_person_record_status`, mantendo `Matricula` unica por tenant, `CPF` unico quando preenchido, `Telefone` opcional e `Tipo` obrigatorio somente para cargos operacionais especificos.
...
- `CPF invalido. Informe 11 digitos ou deixe em branco.`:
  - Causa: tentativa de cadastrar/editar pessoa com `CPF` preenchido em formato invalido.
  - Solucao: informar CPF valido com 11 digitos ou deixar o campo vazio.
- `Ja existe pessoa com este CPF no tenant atual.`:
  - Causa: tentativa de cadastrar/editar pessoa com `CPF` ja usado no mesmo tenant.
  - Solucao: editar o registro existente ou informar outro CPF.
- `Existem CPFs duplicados em people. Regularize antes de aplicar a trava`:
  - Causa: a migration `199_people_cpf_unique_phone_and_conditional_type.sql` encontrou dados legados com o mesmo CPF no mesmo tenant.
  - Solucao: corrigir/remover as duplicidades no banco e reaplicar a migration.
- `Nenhuma pessoa valida foi encontrada para importar.`:
  - Causa: CSV de cadastro em massa de Pessoas sem linhas validas ou com colunas obrigatorias ausentes.
  - Solucao: baixar o modelo na tela `/pessoas`, preencher `nome`, `matricula`, `cargo` e `tipo`, e baixar o CSV de erros para corrigir as linhas rejeitadas.
