...
- `src/app/`
  ...
  - `(dashboard)/medicao/page.tsx`: rota da tela de Medicao com `cadastro + filtros + lista` paginada, filtro por Tipo de Servico vinculado ao projeto, modos `Com producao` e `Sem producao`, persistencia transacional em banco, cadastro independente da programacao, match automatico por `Projeto + Equipe + Data`, sugestao automatica de taxa pela ultima medicao do projeto, `Status execucao` baseado no ultimo `Estado Trabalho` do projeto, indicador de composicao da equipe na data na lista/detalhes e card de Garantia de faturamento minimo.
...
- `api/stock-balance/route.ts`: lista o saldo atual por centro/material com filtros, paginacao server-side, exclui centros de equipe da tela de Estoque Atual, recompõe materiais historicos com saldo `0` nos centros fisicos quando necessario e mantem historico enriquecido com `Equipe`/`Encarregado`, incluindo filtro por operacao/origem para localizar `Retorno de campo` via `CAMPO / INSTALADO`.
  ...
  - `api/dashboard-medicao/route.ts`: consolida indicadores de Medicao por ciclo, status economico, Pendencias, Parcial planejado beneficio atingido, meta, encarregado e supervisor, monta semanas sequenciais do ciclo 21 a 20, aplica filtro de projeto por SOB, calcula Concluidos x Parciais x Parcial planejado beneficio atingido x Pendencias do ciclo e da `Visao geral por periodo`, inclui Garantia de faturamento minimo no recorte `De/Para` a partir de ordens Sem producao com valor positivo no cabecalho, calcula projetos distintos, ticket medio por projetos, ticket medio por servicos e lista de projetos por categoria/equipe/encarregado/supervisor com valor cobrado e centro, calcula dias trabalhados por datas distintas de execucao em medicoes Com producao, consolida a meta oficial por `team_id`, rateia as tres metas gerenciais somente entre encarregados salvos nas medicoes validas conforme seus dias medidos por equipe, soma producao por supervisor vinculado as equipes, calcula meta de supervisor por equipes que produziram e por todas as equipes ativas vinculadas, retorna consolidacoes por ciclo e por semana, calcula a projecao de fechamento do ciclo e filtra tudo pelo tenant autenticado.
...
- `src/modules/dashboard/medicao/`
  - `MeasurementPageView.tsx`: tela de Ordem de Medicao com cadastro independente da programacao, lista paginada, filtro por Tipo de Servico do projeto, modos `Com producao` e `Sem producao`, motivo estruturado por tenant, inclusao de atividades da medicao, taxa unica por ordem com coluna `Taxa aplicada` na edicao, sugestao automatica da taxa ao selecionar projeto (ultima medicao do projeto), cadastro em massa CSV com suporte aos dois tipos, detalhe por item com `taxa` visivel, importacao reforcada por match exato/univoco do codigo da atividade, bloqueio de atividade duplicada na mesma ordem com validacao tambem na RPC, status de execucao baseado no ultimo `Estado Trabalho` do projeto, coluna de composicao da equipe na data e resumo filtrado da garantia minima.
...
35. A rota `/medicao` opera `Ordem de Medicao` com persistencia transacional via `/api/medicao`, lista paginada no servidor, filtros por `Tipo de Servico` do projeto, `Tipo` e `Motivo sem producao`, modos `Com producao` e `Sem producao`, motivo estruturado por tenant para ordens sem producao, taxa unica por ordem reaplicada a todos os itens na edicao, sugestao automatica de taxa por projeto (ultima medicao), match automatico opcional por `Projeto + Equipe + Data`, indicador que informa na lista e nos detalhes se existe composicao ativa da equipe na data de execucao, card filtrado de Garantia de faturamento minimo, historico, exportacoes e cadastro em massa CSV com suporte aos dois tipos de ordem.
