...
| Data | O que mudou |
|---|---|
| 2026-09-14 | Checagem de permissão por página também separa "sem permissão" de "Supabase fora": `checkPageReadAccess` (`src/lib/server/pageAuthorization.ts`) substitui 8 cópias de `ensure*PageAccess` (dash-estoque, dash-operacional-faturamento, consumo-projeto, estornos, estoque-equipes, 3 rotas de medicao-asbuilt) e devolve `unavailable` → 503; `requirePageAction` responde 503 (não 500) quando a consulta falha por rede/timeout/5xx. Regra de acesso dessas 8 rotas inalterada — continua sem o gate `default_user_access` do `requirePageAction` (divergência registrada) |
