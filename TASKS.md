...
- [ ] [P1][Performance][Dashboard Medicao/Equipes] Evitar segunda chamada automatica no mount quando a primeira resposta preencher `startDate`/`endDate`/`selectedCycleStart` ou `cycleStart`; preservar refresh manual por filtros.
- [ ] [P1][Performance][Dash Operacional/Faturamento] Consolidar carga inicial de metadados e valores de projetos para reduzir chamadas simultaneas ao mesmo endpoint e evitar reprocessamento pesado sem acao do usuario.
