- [x] [Melhoria][Governanca] Versionar `AGENTS.md` e a pasta `verificação/` (renomeada para `verificacao/`) que hoje existem so localmente por causa do `.gitignore`: secao "# Database" + `verificação/` removida do `.gitignore` (inclui entrada morta `Recuperar.txt`); 12 arquivos numerados fundidos em `guias/guia_backend.md`, `guias/guia_frontend.md`, `guias/guia_sql.md` e `guias/guia_validacao.md`, logs historicos preservados em `_archive/verificacao-logs/`, `crc/*.md` mantido versionado em `verificacao/crc/`. Parte da reestruturacao de documentacao de agentes (CLAUDE.md/AGENTS.md/guias/prompts), 2026-07. Achado original: `docs/Auditoria_Lixo_NextSupabase_2026-07.txt` (A1/A3/M3).
- [x] [Melhoria][Documentacao] Resolver referencia quebrada a `REGRA_DE_NEGOCIO.md` em CLAUDE.md/AGENTS.md: referencia removida da hierarquia de precedencia (secao 2 do novo `CLAUDE.md`); arquivo nao criado ainda — consolidacao de regras de negocio dispersas em `verificacao/crc/*.md` e docs de produto fica registrada como TODO em tarefa propria. Achado original: `docs/Auditoria_Lixo_NextSupabase_2026-07.txt` (A2).
- [ ] [Melhoria][Seguranca] Definir destino oficial de `.env.backup_supabase` (contem nomes de chave de `SUPABASE_SERVICE_ROLE_KEY`, `DB_PASSWORD`, `PGPASSWORD`, `CRON_SECRET` etc. em texto puro na raiz do projeto, nao commitado mas em disco) e mover `Backup/` (15MB) para fora do diretorio de trabalho do repositorio. Achado em `docs/Auditoria_Lixo_NextSupabase_2026-07.txt` (A4/M1).
- [ ] [Melhoria][Dependencias] Atualizar `@supabase/supabase-js` (2.98.0 -> 2.110.2) em commit isolado com validacao de build/typecheck; demais pacotes com defasagem menor (`next`, `react`, `eslint`, `@types/node`). Achado em `docs/Auditoria_Lixo_NextSupabase_2026-07.txt` (M4).
