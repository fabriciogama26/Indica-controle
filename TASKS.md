...
- [ ] [Programacao][Etapas] Projetar renumeracao automatica transacional de ETAPAs ao inserir/reprogramar uma programacao entre datas ja existentes do mesmo projeto/equipe/grupo: exemplo 08/07 etapa 1, 10/07 etapa 2, 20/07 etapa 3; ao inserir 15/07, persistir 15/07 etapa 3 e deslocar futuras para etapa 4+. Precisa migration/RPC com lock por `tenant_id + project_id`, atualizar `programming_group_id`, registrar historico e respeitar `ETAPA UNICA`, `ETAPA FINAL`, `CONCLUIDO`/`ANTECIPADO` e validacoes multi-tenant.
