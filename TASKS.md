- [x] [Visualizacao Programacao][Calendario] Desacoplar o `Calendario Semanal de Programacao` do card `Filtros`: `weeklySchedules` passa a derivar de `schedules` recortado apenas pelo intervalo da semana (antes derivava de `filteredSchedules`), `calendarTeams` volta a listar todas as equipes do tenant em vez de recortar pelo filtro de `Equipe`, e `applyFilters`/`clearFilters` deixam de mover a semana exibida — a semana so muda por `Anterior`/`Semana atual`/`Proxima`. A lista continua obedecendo todos os filtros. `fetchBoardSnapshot` ja busca a uniao entre periodo filtrado e periodo da semana, entao nao ha lacuna de dados. Validado com `npx tsc --noEmit` e `npm run lint` (0 erros; warnings legados fora do escopo). Docs: `docs/Tela_Visualizacao_Programacao_SaaS.txt`.
