...
- [x] Adicionar painel de prazos na Programacao Simples acima do cadastro, com cards-resumo (prioridade `Vence hoje`, `Vence em breve`, `Vencida`, `No prazo`) e cards por `SOB` em carrossel horizontal (6 por pagina, setas esquerda/direita) para obras com `Data limite <= hoje + 15 dias`, ignorando projetos com `Estado Trabalho = CONCLUIDO` e hierarquia visual (`>=30 dias` vermelho escuro, atrasada vermelho, hoje destaque, em breve laranja).
