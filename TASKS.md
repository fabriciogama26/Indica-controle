- [x] [Almoxarifado][Enderecamento][UI] Trocar o pontinho de ocupacao por andar (3 cores fixas, sem numero) por um selo com a fracao real "ocupado/total" e cor de fundo gradual (`color-mix`, claro->azul solido); nova `floorOccupancyCounts` em `utils.ts` no lugar de `floorOccupancyStatus`. Busca por material (`/mapa-almoxarifado`) passou a apagar/dessaturar celulas sem correspondencia (modo "holofote") em vez de so marcar a combinacao com borda azul. Validado com `npx tsc --noEmit` e `npx eslint`.
- [x] [Almoxarifado][Enderecamento][Bug] Corrigir lista "Materiais para enderecar" (`/mapa-almoxarifado`, antes "Sem endereco"): material sumia da lista assim que ganhava 1 endereco, impedindo endereca-lo numa segunda posicao (o proprio proposito da feature de multi-posicao). Lista agora mostra todo material com saldo > 0 independente de ja ter endereco, com "Ja enderecado em N posicao(oes)" indicado por linha. Validado com `npx tsc --noEmit` e `npx eslint`.
- [x] [Almoxarifado][Enderecamento][Feature] Permitir o mesmo material em mais de uma posicao do mapa (endereco = marcador de presenca por posicao, sem quantidade): migration 292 remove `warehouse_material_addresses_unique_material`, recria `assign_warehouse_material_address` (com `p_address_id` opcional: nulo cria endereco novo, preenchido edita/realoca um endereco especifico), `assign_warehouse_material_addresses_batch` (sem bloqueio de material repetido/ja enderecado) e `clear_warehouse_material_address` (por `p_address_id`). Backend (`handleWarehouseMapGet`/`handleWarehouseAddressPost`/`handleWarehouseAddressDelete`) e frontend (`WarehouseAddressingMapPageView.tsx`) reescritos para `materiais[].enderecos: []` (lista) no lugar de campos soltos por material; novo botao "Adicionar outro endereco"; "Realocar"/"Remover endereco" passam a operar por endereco especifico, nao mais pelo material inteiro. Decisao consciente de nao rastrear quantidade por posicao (mexer no saldo via Entrada/Saida/Transferencia nao sabe de prateleira, entao uma quantidade manual ficaria desatualizada). Validado com `npx tsc --noEmit` e `npx eslint`; nao testado no navegador nesta sessao (sem ambiente Supabase linkado).
- [x] [Almoxarifado][Enderecamento][UI] Trocar o `window.confirm` nativo do "Limpar posicao" (`/mapa-almoxarifado`) por um modal proprio (`clearCellConfirm`), no mesmo padrao visual dos demais modais do modulo, com botoes `Cancelar`/`Remover endereco`. Validado com `npx tsc --noEmit` e `npx eslint`.
- [x] [Almoxarifado][Enderecamento][Bug] Corrigir modal "Atribuir endereco" (`/mapa-almoxarifado`) aceitando `Andar`/`Posicao` fora do configurado na prateleira/baia/pallet: campos eram `<input type="number">` sem `max`, so bloqueados pelo backend (`INVALID_ADDRESS`) no submit. Viraram `<select>` com os andares/posicoes reais da prateleira, reenquadrando a posicao automaticamente ao trocar prateleira/andar. Adicionado `clamp` compartilhado em `utils.ts` (usado tambem em `WarehouseMapConfigPageView.tsx`, substituindo a copia local). Validado com `npx tsc --noEmit` e `npx eslint`.
- [x] [Almoxarifado][Enderecamento][UI] Mover o botao `Salvar` da Configuracao do Mapa do painel "Prateleira selecionada" para o cabecalho do card "Configuracao do mapa", ao lado de `Historico` (usa `form="warehouse-map-config-form"` para continuar submetendo o mesmo formulario). Validado com `npx tsc --noEmit` e `npx eslint`.
- [x] [Almoxarifado][Enderecamento][Fix] Corrigir mensagem do historico da Configuracao do Mapa: distinguir "Configuracao salva sem alteracoes de layout" (snapshot presente, usuario clicou Salvar sem editar nada) de "registro anterior a este recurso" (sem snapshot, migration antiga). Validado com `npx tsc --noEmit` e `npx eslint`.
- [x] [Almoxarifado][Enderecamento][Performance] Criar migration 291 com indice composto `(tenant_id, map_id, action_type, created_at desc)` em `warehouse_address_history`, substituindo o indice sem `action_type`: evita variedade de scan quando o historico paginado de `CONFIG_SAVE` precisa pular o volume, maior e crescente, de `ADDRESS_ASSIGN`/`ADDRESS_CLEAR` do mesmo mapa.
- [x] [Almoxarifado][Enderecamento][UI] Trocar a cor da celula `Prateleira` de branco para lilas (`#f6f0fc`) no grid de `/configuracao-mapa-almoxarifado` (e reflexo em `/mapa-almoxarifado`, CSS compartilhado), diferenciando de Pallet (verde) e Baia (azul).
- [x] [Almoxarifado][Enderecamento][Feature] Tornar automatico o recarregamento da Configuracao do Mapa quando o salvar falha com `CONCURRENT_MODIFICATION`: a tela busca a versao atual sozinha (sem exigir clique) e nunca reenvia as edicoes antigas por cima da versao nova (evita sobrescrever silenciosamente a mudanca de outro usuario); usuario so precisa refazer as alteracoes e salvar de novo. Validado com `npx tsc --noEmit` e `npx eslint`.
- [x] [Almoxarifado][Enderecamento][Feature] Criar historico paginado (5/pagina) da Configuracao do Mapa do Almoxarifado: migration 290 grava snapshot `before`/`after` (colunas, linhas, prateleiras/andares) em `warehouse_address_history.details` a cada `CONFIG_SAVE`; novo `GET /api/warehouse-addressing/config/history` (`handleWarehouseConfigHistoryGet`) resolve quem alterou via `app_users`; botao `Historico` em `/configuracao-mapa-almoxarifado` abre modal com resumo legivel do que mudou (`summarizeConfigHistoryChanges`). Validado com `npx tsc --noEmit` e `npx eslint`.
- [x] [Almoxarifado][Enderecamento][Feature] Adicionar limpeza em lote por posicao (coluna+linha) do mapa: RPC `clear_warehouse_cell_addresses` (migration 289) remove de uma vez todos os materiais enderecados numa celula, independente de andar/posicao/tipo; botao `Limpar posicao` em `/mapa-almoxarifado` na celula selecionada, para uso antes de mover uma prateleira/baia/pallet na configuracao. Validado com `npx tsc --noEmit` e `npx eslint`.
- [x] [Almoxarifado][Enderecamento][Feature] Detalhar conflitos ao salvar configuracao do mapa: `save_warehouse_map_config` (migration 289) passa a retornar `conflicts` (material/posicao) junto do erro `ADDRESSES_OUTSIDE_NEW_LAYOUT`; `/configuracao-mapa-almoxarifado` exibe essa lista em tela para orientar limpeza/realocacao antes de tentar salvar de novo. Validado com `npx tsc --noEmit` e `npx eslint`.
- [x] [Bug][Dashboard Medicao] Corrigir falha "Falha ao carregar valores das medicoes" ao carregar recortes com muitas ordens: a consulta de `project_measurement_order_items` agora e feita em chunks de 200 IDs por `measurement_order_id`, mantendo filtro por `tenant_id`, colunas explicitas e retorno sem alterar o contrato visual.
- [x] [Almoxarifado][Enderecamento][Fix] Limitar layout do mapa no frontend e banco: 15 colunas, 20 linhas, 10 andares e 10 posicoes por andar.
- [x] [Almoxarifado][Enderecamento][Feature] Adicionar `PALLET` ao cadastro do mapa e enderecamento em massa atomico no mapa para materiais com saldo positivo e sem endereco, limitado a 100 itens por lote.
- [x] [Almoxarifado][Enderecamento][Fix] Identificar centro fisico de almoxarifado por `stock_centers` ativo/OWN/controla saldo sem vinculo em `teams.stock_center_id`, bloqueando centros `EQUIPE - ...` no backend, na RPC e no trigger do mapa.
- [x] [Almoxarifado][Enderecamento][Fix] Compatibilizar a migration 288 com bancos sem colunas granulares em `role_page_permissions`/`app_user_page_permissions`, criando/backfillando `can_create`, `can_update`, `can_cancel`, `can_reverse`, `can_import` e `can_export` antes dos inserts de permissoes das novas telas.
- [x] [Almoxarifado][Enderecamento] Etapa 1 funcional: criar schema/RPCs de enderecamento (`warehouse_maps`, `warehouse_shelves`, `warehouse_shelf_floors`, `warehouse_material_addresses`, `warehouse_address_history`), limites `stock_minimum`/`stock_maximum` em Materiais, APIs `/api/warehouse-addressing/config` e `/api/warehouse-addressing/map`, telas reais `/configuracao-mapa-almoxarifado` e `/mapa-almoxarifado`, menu/permissoes/docs/README atualizados; validado com `npx tsc --noEmit` e `npm run lint`.
- [ ] [Almoxarifado][Enderecamento] Validar migration 288 em ambiente Supabase correto antes de deploy (`npm run db:check-link`, `npm run db:migration-list`, `npm run db:lint`) sem aplicar automaticamente se o project ref divergir.
- [ ] [Almoxarifado][Enderecamento] Etapa 2: QA visual no navegador para desktop/mobile, ajuste fino de UX de atribuicao por posicao, mensagens de conflito 409 e conferencia de payload/egress acima de 100KB.
- [x] [Exportacao][Refactor] Fase 1 (PR8) do plano de paginacao/exportacao CONCLUIDO: migrar Dashboard Medicao (`DashboardMeasurementPageView.tsx`), ultimo dos 4 dashboards planejados, para `CsvExportButton` (com `showProgressModal={false}`) no botao de exportacao do modal de detalhe de projetos. Testado no navegador e aprovado pelo usuario. Fecha o PR8: Dash Estoque, Dash Operacional Faturamento, Dashboard Equipes e Dashboard Medicao migrados, cada um em commit e branch dedicados.
- [x] [Exportacao][Refactor] Fase 1 (PR8) do plano de paginacao/exportacao: migrar Dashboard Equipes (`DashboardTeamsPageView.tsx`) para `CsvExportButton` (com `showProgressModal={false}`) nos 2 botoes de exportacao dos modais de detalhe (projetos e contribuicoes por equipe). Testado no navegador e aprovado pelo usuario.
- [x] [Exportacao][Refactor] Fase 1 (PR8) do plano de paginacao/exportacao: migrar Dash Operacional Faturamento (`OperationalBillingDashboardPageView.tsx`) para `CsvExportButton` (com `showProgressModal={false}`) nos 4 botoes de exportacao (tabela principal, resumo por categoria, valores por projeto, modal de projetos do indicador). Testado no navegador e aprovado pelo usuario.
- [x] [Exportacao][Refactor] Fase 1 (PR8) do plano de paginacao/exportacao: migrar Dash Estoque (`StockDashboardPageView.tsx`) para `CsvExportButton` (com `showProgressModal={false}`, reaproveitando o `ExportProgressModal` ja existente na tela) nos 2 botoes de exportacao do grafico de dispersao (normal e ampliado). Testado no navegador e aprovado pelo usuario.
- [x] [UI][Refactor] Fase 1 (PR7) do plano de paginacao/exportacao CONCLUIDO: migrar Composicao-Equipe (`TeamCompositionPageView.tsx`), ultima das 14 telas planejadas, para `usePagination`/`Pagination` compartilhados. Testado no navegador e aprovado pelo usuario. Fecha o PR7: Equipes, Medicao, Pessoas, Atividades, Materiais, Cargo, Projetos, Posicao Trafo, Estoque Corrente, Medicao As Built, Faturamento, Estornos, Estoque-Equipes e Composicao-Equipe migradas, cada uma em commit e branch dedicados, com teste manual no navegador aprovado pelo usuario em todas.
- [x] [UI][Refactor] Fase 1 (PR7) do plano de paginacao/exportacao: migrar Estoque-Equipes (`TeamStockPageView.tsx`) para `usePagination`/`Pagination` compartilhados com `showTotal={false}`. Testado no navegador e aprovado pelo usuario.
- [x] [UI][Refactor] Fase 1 (PR7) do plano de paginacao/exportacao: migrar Estornos (`ReversalsPageView.tsx`) para o componente `Pagination` compartilhado. `Pagination` ganhou prop opcional `label` para telas com rodape divergente (aqui: "Pagina X de Y | Z registro(s)" e botoes sem classe propria). Removidas `canGoPrevious`/`canGoNext`, agora cobertas pelo `disabled`/comparacao interna do componente. Testado no navegador e aprovado pelo usuario.
- [x] [UI][Refactor] Fase 1 (PR7) do plano de paginacao/exportacao: migrar Faturamento (`BillingPageView.tsx`) para o componente `Pagination` compartilhado com `showTotal={false}` (mesmo padrao de Medicao As Built: estado em objeto unico `pagination`, botoes chamam `loadOrders` diretamente). Testado no navegador e aprovado pelo usuario.
- [x] [UI][Refactor] Fase 1 (PR7) do plano de paginacao/exportacao: migrar Medicao As Built (`AsbuiltMeasurementPageView.tsx`) para o componente `Pagination` compartilhado (sem o hook `usePagination`, ja que o estado aqui e um unico objeto `pagination: {page, pageSize, total}` atualizado a cada fetch, e os botoes chamam `loadOrders` diretamente em vez de so trocar estado). `Pagination` ganhou prop opcional `showTotal` (default `true`) para preservar o rodape original desta tela, que nao exibe "Total" (o total ja aparece em outro card). Testado no navegador e aprovado pelo usuario.
- [x] [UI][Refactor] Fase 1 (PR7) do plano de paginacao/exportacao: migrar Estoque Corrente (`CurrentStockPageView.tsx`) para `usePagination`/`Pagination` compartilhados. Testado no navegador e aprovado pelo usuario.
...
- [x] [UI][Refactor] Fase 1 (PR7) do plano de paginacao/exportacao: migrar Atividades (`ActivitiesPageView.tsx`) para `usePagination`/`Pagination` compartilhados. Testado no navegador e aprovado pelo usuario.
- [x] [UI][Refactor] Fase 1 (PR7) do plano de paginacao/exportacao: migrar Pessoas (`PeoplePageView.tsx`) para `usePagination`/`Pagination` compartilhados, reaproveitando o CSS Module existente. Testado no navegador e aprovado pelo usuario.
