...
import {
  isMatriculationNumericTypeMismatchError,
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (normalizeText(body.action).toUpperCase() === "BATCH_IMPORT") {
      const authorizationError = await authorizePageAction(resolution, "pessoas", "import");
    ...
    const authorizationError = await authorizePageAction(resolution, "pessoas", "create");
    if (authorizationError) {
      return authorizationError;
    }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const authorizationError = await authorizePageAction(resolution, "pessoas", "update");
    if (authorizationError) {
      return authorizationError;
    }
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const authorizationError = await authorizePageAction(resolution, "pessoas", action === "ACTIVATE" ? "update" : "cancel");
    if (authorizationError) {
      return authorizationError;
    }
