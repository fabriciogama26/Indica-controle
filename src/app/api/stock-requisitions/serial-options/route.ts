...
// Serial disponivel para atendimento de requisicao: sempre a partir do centro principal (origem da REQUISITION).
export async function GET(request: NextRequest) {
  try {
    ...
    const [materialResult, stockCenterResult] = await Promise.all([
      supabase
        .from("materials")
    ...
    if (materialResult.error || stockCenterResult.error) {
    ...
    if (!material?.is_active || !isSerialTrackedMaterial(serialTrackingType) || !stockCenter?.is_active || stockCenter.center_type !== "OWN") {
      return NextResponse.json({ items: [] });
    }
    ...
    let query = supabase
      ...
      .eq("current_stock_center_id", stockCenterId)
      .order("serial_number", { ascending: true })
      .limit(pageSize);

    if (serialNumber) query = query.ilike("serial_number", `%${serialNumber}%`);
    if (requiresLotCode(serialTrackingType) && lotCode) query = query.ilike("lot_code", `%${lotCode}%`);
