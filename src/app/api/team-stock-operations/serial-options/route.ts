...
import { normalizeText } from "@/lib/server/stockTransfers";
import { normalizeTeamOperationKind } from "@/lib/server/teamStockOperations";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const operationKind = normalizeTeamOperationKind(request.nextUrl.searchParams.get("operationKind"));
    ...
    if (!operationKind || !stockCenterId || !teamId || !materialId) {
      return NextResponse.json({ items: [] });
    }

    if (operationKind === "FIELD_RETURN") {
      return NextResponse.json({ items: [] });
    }

    const [materialResult, teamResult, stockCenterResult, teamStockCenterConflictResult] = await Promise.all([
      supabase
        .from("materials")
      ...
      supabase
        .from("teams")
        .select("id, stock_center_id, ativo")
        .eq("tenant_id", appUser.tenant_id)
        .eq("id", teamId)
        .maybeSingle<TeamRow>(),
      supabase
    ...
    if (materialResult.error || teamResult.error || stockCenterResult.error || teamStockCenterConflictResult.error) {
    ...
    const sourceStockCenterId = operationKind === "REQUISITION" ? stockCenterId : team.stock_center_id;
    ...
    let query = supabase
      ...
      .eq("current_stock_center_id", sourceStockCenterId)
