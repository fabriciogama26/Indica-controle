...
export async function GET(request: NextRequest) {
  try {
    const resolution = await resolveAuthenticatedAppUser(request, {
      invalidSessionMessage: "Sessao invalida para pesquisar materiais previstos do projeto.",
    ...
    if (error) {
      return NextResponse.json({ message: "Falha ao pesquisar materiais previstos do projeto." }, { status: 500 });
    }
