...
import styles from "./BillingPageView.module.css";
...
export function BillingPageView() {
  ...
  const [filters, setFilters] = useState<BillingFilters>(INITIAL_FILTERS);
  ...
  const [detailOrder, setDetailOrder] = useState<BillingDetail | null>(null);
  const [historyOrder, setHistoryOrder] = useState<BillingListItem | null>(null);
  ...
  const [massImportIssues, setMassImportIssues] = useState<BillingImportIssue[]>([]);
  ...
  async function loadMeta() {
    ...
    try {
      ...
      await logError("Falha ao carregar metadados do faturamento", error);
    } finally {
  ...
  async function loadOrders(page = pagination.page) {
    ...
    try {
      const params = new URLSearchParams({
        page: String(page),
        pageSize: String(BILLING_PAGE_SIZE),
      });
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(async () => {
      try {
        const response = await fetch(`/api/faturamento/activities/catalog?q=${encodeURIComponent(query)}`, {
          headers: authHeaders,
          signal: controller.signal,
        });
        const payload = (await response.json().catch(() => ({}))) as BillingCatalogResponse;
        if (response.ok) {
          setActivityOptions(payload.items ?? []);
        }
      } catch (error) {
        if (!controller.signal.aborted) {
          await logError("Falha ao pesquisar atividade do faturamento", error, { query });
        }
      }
    }, 250);
  ...
  async function addActivity() {
    ...
    if (form.items.some((item) => item.activityId === activity.id)) {
      setError("A mesma atividade nao pode ser repetida no faturamento.");
      return;
    }

    const nextItem: BillingFormItem = {
      rowId: buildRowId(),
  ...
  function updateRow(rowId: string, field: keyof Pick<BillingFormItem, "quantity" | "rate" | "observation">, value: string) {
    setForm((current) => ({
      ...current,
  ...
  async function submitOrder(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : "Falha ao salvar faturamento.");
      await logError("Falha ao salvar faturamento", error, { formId: form.id });
    } finally {
      setIsSubmitting(false);
    }
  }

  async function fetchDetail(order: BillingListItem) {
  ...
  async function openDetail(order: BillingListItem) {
    try {
      setDetailOrder(await fetchDetail(order));
    } catch (error) {
      setError(error instanceof Error ? error.message : "Falha ao carregar detalhes.");
      await logError("Falha ao carregar detalhe do faturamento", error, { orderId: order.id });
    }
  }

  async function startEdit(order: BillingListItem) {
    try {
      ...
      await logError("Falha ao iniciar edicao do faturamento", error, { orderId: order.id });
    }
  }

  async function openHistory(order: BillingListItem) {
    try {
      ...
      await logError("Falha ao carregar historico do faturamento", error, { orderId: order.id });
    }
  }
  ...
  async function exportOrders() {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : "Falha ao exportar faturamentos.");
      await logError("Falha ao exportar faturamento", error, { filters });
    } finally {
  ...
  async function exportOrdersDetailed() {
    ...
    try {
      ...
      if (!details.length) {
        throw new Error("Falha ao carregar detalhes dos faturamentos para exportar.");
      }
      ...
      if (failedCount > 0) {
        setSuccess(`Detalhamento exportado com sucesso. ${failedCount} faturamentos foram ignorados por falha ao carregar detalhes.`);
      }
    } catch (error) {
      setError(error instanceof Error ? error.message : "Falha ao exportar detalhamento.");
      await logError("Falha ao exportar detalhamento do faturamento", error, { filters });
    } finally {
  ...
  function downloadMassErrors() {
    downloadCsv("erros_faturamento.csv", [
  ...
  async function lookupActivityByCode(code: string) {
    const response = await fetch(`/api/faturamento/activities/catalog?q=${encodeURIComponent(code)}&includeInactive=true`, { headers: authHeaders });
    const payload = (await response.json().catch(() => ({}))) as BillingCatalogResponse;
  ...
  async function importMassFile(file: File) {
    ...
    try {
      ...
      for (let index = 1; index < rows.length; index += 1) {
        ...
        const kindInput = readCsvField(row, headerMap, "tipo_faturamento");
        ...
        if (group.items.some((item) => item.activityId === activity.id)) {
          issues.push({ linha: rowNumber, coluna: "codigo_atividade", valor: activityInput, erro: "Atividade duplicada no mesmo faturamento." });
          continue;
        }
      ...
      await logError("Falha no cadastro em massa do faturamento", error);
    } finally {
  ...
  return (
    <section className={styles.wrapper}>
      <article className={`${styles.card} ${form.id ? styles.editingCard : ""}`}>
        <h2 className={styles.cardTitle}>{form.id ? "Editar Faturamento" : "Cadastro de Faturamento"}</h2>
        <form id="billing-form" className={styles.formGrid} onSubmit={submitOrder}>
          ...
          <label className={styles.field}>
            ...
            <select
              ...
              onChange={(event) => setForm((current) => ({
                ...current,
                billingKind: event.target.value as BillingKind,
          ...
          <label className={styles.field}>
            ...
            <select
              ...
              disabled={form.billingKind !== "SEM_PRODUCAO"}
        ...
        <div className={styles.subCard}>
          ...
          <div className={styles.inlineForm}>
            <label className={styles.field}>
              <span>Atividade</span>
              <input list="billing-activities" value={form.activitySearch} onChange={(event) => setForm((current) => ({ ...current, activitySearch: event.target.value }))} />
            </label>
          ...
          <div className={styles.actions}>
            <button type="submit" form="billing-form" className={styles.primaryButton} disabled={isSubmitting}>{isSubmitting ? "Salvando..." : "Salvar faturamento"}</button>
      ...
      <article className={styles.card}>
        ...
        <div className={styles.filterGrid}>
          <label className={styles.field}><span>Projeto</span><input list="billing-projects" value={filterDraft.projectSearch} onChange={(event) => updateFilterProjectFromInput(event.target.value)} /></label>
          <label className={styles.field}><span>Status</span><select value={filterDraft.status} onChange={(event) => setFilterDraft((current) => ({ ...current, status: event.target.value as BillingFilters["status"] }))}><option value="TODOS">Todos</option><option value="ABERTA">Aberta</option><option value="FECHADA">Fechada</option><option value="CANCELADA">Cancelada</option></select></label>
          <label className={styles.field}><span>Tipo</span><select value={filterDraft.billingKind} onChange={(event) => setFilterDraft((current) => ({ ...current, billingKind: event.target.value as BillingFilters["billingKind"] }))}><option value="TODOS">Todos</option><option value="COM_PRODUCAO">Com producao</option><option value="SEM_PRODUCAO">Sem producao</option></select></label>
      ...
      <article className={styles.card}>
        <div className={styles.tableHeader}>
          <h2 className={styles.cardTitle}>Lista de Faturamentos</h2>
        ...
        <div className={styles.summaryBar}>
          <div><span>Faturamentos na pagina</span><strong>{orders.length}</strong></div>
            ...
            <tbody>
              {orders.length ? orders.map((order) => (
                <tr key={order.id}>
                  ...
                  <td><span className={order.status === "ABERTA" ? styles.statusTag : styles.statusTagDanger}>{billingStatusLabel(order.status)}</span></td>
        ...
        <div className={styles.modalOverlay} onClick={() => setDetailOrder(null)}>
          <article className={styles.modalCard} role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}>
            <header className={styles.modalHeader}>
              <div><h4>Detalhes do Faturamento</h4><p className={styles.modalSubtitle}>ID: {detailOrder.id}</p></div>
      ...
      {historyOrder ? (
        <div className={styles.modalOverlay} onClick={() => setHistoryOrder(null)}>
          <article className={styles.modalCard} role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}>
            <header className={styles.modalHeader}>
              <div><h4>Historico do Faturamento</h4><p className={styles.modalSubtitle}>{historyOrder.billingNumber}</p></div>
      ...
      {isMassImportOpen ? (
        <div className={styles.modalOverlay} onClick={() => setIsMassImportOpen(false)}>
          <article className={styles.modalCard} role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}>
            <header className={styles.modalHeader}>
              <div><h4>Cadastro em massa</h4><p className={styles.modalSubtitle}>Importe um CSV para criar faturamentos em lote.</p></div>
