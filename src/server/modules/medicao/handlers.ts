// Handlers HTTP de Medicao compartilhados entre `/api/medicao` (tecnica) e
// `/api/medicao-comercial`. As duas rotas so delegam: a diferenca entre elas e o
// `MeasurementRouteConfig` recebido (page_key da tela e categoria de equipe).
...
import type { MeasurementOrderStatus, SaveMeasurementBatchPayload, SaveMeasurementBatchRpcResult, SaveMeasurementPayload, SaveMeasurementRpcResult, SetMeasurementStatusRpcResult, UpdateStatusPayload } from "./types";
import { parseMeasurementOrderListFilters, findDuplicateMeasurementActivityId, normalizeIsoDate, normalizeMeasurementItems, normalizeMeasurementKind, normalizePositiveIntegerArray, normalizePositiveNumber, normalizeText, normalizeTimeOfDay, normalizeUuid } from "./normalizers";
import { fetchMeasurementOrderDetail, loadHistory, measurementModuleMigrationHint } from "./queries";
...
export async function handleMeasurementGet(request: NextRequest, config = DEFAULT_MEASUREMENT_ROUTE_CONFIG) {
  ...
  const authorizationError = await authorizeMeasurementVariantReadOrExportAction(resolution, "read", config.commercial);
  ...
  if (historyOrderId) {
    const isAllowedOrder = await orderMatchesRouteCategory({
  ...
  const listResult = await listMeasurementOrdersPage({
    ...
    tenantId: resolution.appUser.tenant_id,
    ...parsed.filters,
    teamCategoryCodeFilter: config.teamCategoryCode,
    page,
    pageSize,
  });
...
async function saveMeasurementOrder(request: NextRequest, method: "POST" | "PUT", config = DEFAULT_MEASUREMENT_ROUTE_CONFIG) {
  const resolution = await resolveAuthenticatedAppUser(request, {
  ...
  const authorizationError = await authorizePageAction(resolution, config.pageKey, method === "POST" ? "create" : "update");
  if (authorizationError) {
    return authorizationError;
  }
  ...
  const commercialEmployee1Id = normalizeUuid(payload?.commercialEmployee1Id);
  const commercialEmployee2Id = normalizeUuid(payload?.commercialEmployee2Id);
  const commercialOrderRef = normalizeText(payload?.commercialOrderRef) || null;
  const commercialProcessId = normalizeUuid(payload?.commercialProcessId);
  const commercialStartTime = normalizeTimeOfDay(payload?.commercialStartTime);
  const commercialEndTime = normalizeTimeOfDay(payload?.commercialEndTime);
  ...
  if (teamId) {
    const teamCategoryCode = await resolveTeamCategoryCode({
      supabase: resolution.supabase,
      tenantId: resolution.appUser.tenant_id,
      teamId,
    });
    if (teamCategoryCode !== config.teamCategoryCode) {
      return NextResponse.json(
        { message: config.commercial ? "Selecione uma equipe comercial." : "Selecione uma equipe tecnica." },
        { status: 422 },
      );
    }
  }
  ...
  if (config.commercial) {
    if (!commercialEmployee1Id || !commercialEmployee2Id) {
      return NextResponse.json({ message: "Selecione os dois eletricistas da medicao comercial." }, { status: 400 });
    }

    if (commercialEmployee1Id === commercialEmployee2Id) {
      return NextResponse.json({ message: "Os dois integrantes da medicao comercial devem ser diferentes." }, { status: 400 });
    }

    if (!commercialProcessId) {
    ...
    if (commercialEndTime <= commercialStartTime) {
  ...
  if (measurementKind === "COM_PRODUCAO") {
    // Na Medicao Comercial o Projeto e opcional: a equipe comercial atende
    // demanda que nem sempre tem projeto aberto, e nesse caso a referencia da
    // execucao e o campo `Ordem`. Na Medicao tecnica nada muda.
    if (!projectId && !config.commercial) {
  ...
  const rpcArgs = {
    ...
    // A tela comercial nao trabalha com Programacao: mesmo que o cliente mande um
    // id, o vinculo nao e criado.
    p_programming_id: config.commercial ? null : programmingId,
  ...
  const { data, error } = config.commercial
    ? await resolution.supabase.rpc("save_project_commercial_measurement_order", {
        ...rpcArgs,
        p_commercial_employee_1_person_id: commercialEmployee1Id,
        p_commercial_employee_2_person_id: commercialEmployee2Id,
        p_commercial_order_ref: commercialOrderRef,
        p_commercial_process_id: commercialProcessId,
        p_commercial_start_time: commercialStartTime,
        p_commercial_end_time: commercialEndTime,
      })
...
async function saveMeasurementOrderBatchPartial(request: NextRequest, config = DEFAULT_MEASUREMENT_ROUTE_CONFIG) {
  const resolution = await resolveAuthenticatedAppUser(request, {
  ...
  if (config.commercial) {
    return NextResponse.json({ message: "Cadastro em massa ainda nao esta disponivel para Medicao Comercial." }, { status: 400 });
  }

  const authorizationError = await authorizePageAction(resolution, config.pageKey, "import");
  if (authorizationError) {
    return authorizationError;
  }
  ...
  const uniqueTeamIds = Array.from(new Set(rows.map((row) => row.teamId).filter((item): item is string => Boolean(item))));
  for (const rowTeamId of uniqueTeamIds) {
    const teamCategoryCode = await resolveTeamCategoryCode({
      supabase: resolution.supabase,
      tenantId: resolution.appUser.tenant_id,
      teamId: rowTeamId,
    ...
    if (teamCategoryCode !== config.teamCategoryCode) {
      return NextResponse.json({ message: "Cadastro em massa da Medicao aceita apenas equipes tecnicas." }, { status: 422 });
    }
  }
...
export async function handleMeasurementPost(request: NextRequest, config = DEFAULT_MEASUREMENT_ROUTE_CONFIG) {
  ...
  if (action === "BATCH_IMPORT_PARTIAL") {
    return saveMeasurementOrderBatchPartial(request, config);
  }
  return saveMeasurementOrder(request, "POST", config);
}

export async function handleMeasurementPut(request: NextRequest, config = DEFAULT_MEASUREMENT_ROUTE_CONFIG) {
  return saveMeasurementOrder(request, "PUT", config);
}

export async function handleMeasurementPatch(request: NextRequest, config = DEFAULT_MEASUREMENT_ROUTE_CONFIG) {
  const resolution = await resolveAuthenticatedAppUser(request, {
  ...
  const authorizationError = await authorizePageAction(resolution, config.pageKey, action === "CANCELAR" ? "cancel" : "update");
  if (authorizationError) {
    return authorizationError;
  }
