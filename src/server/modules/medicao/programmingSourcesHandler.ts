// Fontes da Programacao usadas pelas telas de Medicao: projetos, equipes da
// categoria da tela, catalogo de conclusao e etapas programadas.
//
// Na variante comercial nao ha etapa de Programacao a puxar (a Medicao Comercial
// nasce sempre sem programacao), entao `schedules` volta vazio de proposito.
...
async function fetchTeamsByCategory(params: {
  supabase: AuthenticatedAppUserContext["supabase"];
  tenantId: string;
  code: "TECNICA" | "COMERCIAL";
}) {
  ...
  const supervisorIds = Array.from(
    new Set(teams.map((team) => team.supervisor_person_id).filter((value): value is string => Boolean(value))),
  );

  // Equipe comercial nao tem encarregado: o rotulo do select mostra o supervisor,
  // que e o vinculo obrigatorio dessa categoria. O nome vem numa segunda consulta
  // porque o FK de supervisor e composto (id + tenant_id) e o PostgREST nao
  // resolve embed nesse formato.
  const supervisorNames = new Map<string, string>();
  if (supervisorIds.length > 0) {
    const supervisorResult = await params.supabase
      .from("people")
      .select("id, nome")
      .eq("tenant_id", params.tenantId)
      .in("id", supervisorIds)
    ...
    for (const person of supervisorResult.data ?? []) {
      supervisorNames.set(person.id, normalizeText(person.nome));
    }
  }
  ...
  return teams.map((team) => ({
    ...
    foremanName: team.supervisor_person_id ? supervisorNames.get(team.supervisor_person_id) ?? "" : "",
  }));
}

export async function handleMeasurementProgrammingSourcesGet(
  request: NextRequest,
  ...
  try {
    if (config.commercial) {
      const [projects, teams, workCompletionCatalog] = await Promise.all([
