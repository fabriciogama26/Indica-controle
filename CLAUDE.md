...
- `PageView.tsx` até 1.000 linhas; `route.ts`/controller até 1.500 linhas — acima disso exige plano de modularização no TXT da tela. Hoje o limite é violado por 30 arquivos (22 deles `PageView.tsx`) e ainda não há enforcement automático — enquanto o ratchet de baseline não existir, o limite não bloqueia entrega funcional válida, mas todo arquivo acima do teto exige o plano registrado no TXT. Maior concentração atual: `src/server/modules/programacao/handlers.ts` (~2.067 linhas), `src/modules/dashboard/projetos/ProjectsPageView.tsx` (~3.702) e `src/modules/dashboard/medicao/MeasurementPageView.tsx` (~3.327). `src/modules/dashboard/programacao-simples/*` está congelada até a remoção — não recebe refatoração nem crescimento.
