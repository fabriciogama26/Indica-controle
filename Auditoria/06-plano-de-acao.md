...
>
> ⚠️ **2026-09-15 — a 439 estourava o `statement_timeout` em produção** (10.305 ms contra 8s da `service_role`). Causa: `language sql` com CTEs é planejada sem os valores dos parâmetros (plano genérico, custo quadrático). Corrigido pela migration 440 (`plpgsql` + `plan_cache_mode = force_custom_plan`): reproduzido localmente em 10.827 ms → 421 ms, mesmo resultado; **em produção, 10.305 ms → 597 ms** após aplicar a 440. Lição registrada em `guias/guia_sql.md` (regras 28–30): medir a RPC **chamando a função** e contra o timeout da role da API.
