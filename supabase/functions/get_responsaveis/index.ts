// Edge Function: get_responsaveis
// Returns active people allowed as responsavel for the authenticated tenant.
...
serve(async (req) => {
  ...
  const { data, error } = await supabase
    .from('people')
    .select('id, nome, job_titles!inner(code, name)')
    .eq('tenant_id', appUser.tenant_id)
    .eq('ativo', true)
    .in('job_titles.code', ['ENCARREGADO', 'SUPERVISOR'])
