...
-- Agregacoes do Dashboard Estoque (`GET /api/dash-estoque`) numa unica chamada.
--
-- POR QUE UMA RPC
-- ---------------------------------------------------------------------------
-- A rota lia ate 20.000 movimentacoes do periodo e, em chunks de 100 IDs, os itens,
-- as operacoes de equipe, os materiais, os projetos, as equipes e 4 recortes de
-- estorno, para agregar tudo em JavaScript. Um ano de movimentacao virava centenas
-- de consultas encadeadas Vercel -> Supabase (P2.1 de `Auditoria/06-plano-de-acao.md`:
-- ~266 mil chamadas medidas; `loadReversalSets` sozinho, 2 consultas por chunk).
-- Acima de 20.000 movimentacoes a tela recusava carregar (HTTP 422), e as leituras de
-- saldo e de itens sem paginacao podiam ser cortadas em 1.000 linhas pelo PostgREST.
--
-- Aqui: tudo que precisa das linhas de movimentacao/itens/estornos e agregado no banco,
-- sem teto de linhas. O retorno e um unico `jsonb`.
--
-- O QUE FICA NO NODE (de proposito)
-- ---------------------------------------------------------------------------
-- Classificacao e ordenacao de APRESENTACAO sobre a lista de materiais ja agregada
-- (criticos, top saldo, faixas sem giro, Curva ABC, corte por UMB da dispersao). Sao as
-- mesmas funcoes de antes, operando sobre poucas linhas por material, e mantem a
-- ordenacao `localeCompare("pt-BR")` sem depender da collation do banco.
--
-- EQUIVALENCIA COM A IMPLEMENTACAO ANTERIOR (route.ts ate 2026-09-14)
-- ---------------------------------------------------------------------------
-- - Centros fisicos: `stock_centers` ativos, `center_type = 'OWN'`, sem equipe vinculada.
-- - Movimentacao relevante: `entry_date` no periodo, origem OU destino num centro do
--   escopo, e `project_id` quando filtrado.
-- - Transferencia estornada: aparece como original OU como estorno em
--   `stock_transfer_reversals`.
-- - Item estornado: aparece como item original OU item de estorno em
--   `stock_transfer_item_reversals`, numa linha cuja transferencia original ou de estorno
--   esta entre as relevantes.
-- - Tipo da operacao: sem operacao de equipe -> `movement_type`; com operacao ->
--   `operation_kind`; sem `operation_kind` -> REQUISITION se o destino e o centro da equipe,
--   senao RETURN.
-- - Evento: `operation_event_id`, ou a chave `data:equipe:projeto:tipo` quando nulo.
-- - Material com movimento e sem saldo no escopo entra com saldo 0 e preco 0 (como antes).
-- - Opcao de equipe: rotulo da operacao mais recente da equipe (antes: de uma operacao
--   qualquer, sem ordem definida).
--
-- SEGURANCA
-- ---------------------------------------------------------------------------
-- `security invoker` e EXECUTE so para `service_role`: a unica chamadora e a rota, que
-- deriva `p_tenant_id` da sessao e valida a permissao da pagina `dash-estoque` antes.
-- Todas as leituras filtram `tenant_id = p_tenant_id`.
...
comment on function public.get_stock_dashboard_aggregates(uuid, date, date, uuid, uuid, uuid, text, text) is
  'Agregacoes do Dashboard Estoque (saldo por material, evolucao mensal, contagem de operacoes, dispersao e opcoes de filtro) numa unica chamada. Chamada somente pela rota GET /api/dash-estoque com service_role.';
