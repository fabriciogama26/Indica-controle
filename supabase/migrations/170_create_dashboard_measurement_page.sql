-- 170_create_dashboard_measurement_page.sql
-- Cadastra a tela Dashboard Medicao na matriz de paginas e permissoes.

insert into public.app_pages (page_key, path, name, section, description)
values
  ('dashboard-medicao', '/dashboard-medicao', 'Dashboard Medicao', 'Visao Geral', 'Indicadores de medicao por ciclo, meta e encarregado.')
...
join public.app_pages as pages
  on pages.ativo = true
 and pages.page_key = 'dashboard-medicao'
...
with target_pages as (
  ...
  where ativo = true
    and page_key = 'dashboard-medicao'
),
target_users as (
  select
    au.id as user_id,
