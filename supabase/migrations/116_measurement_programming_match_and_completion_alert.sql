-- 116_measurement_programming_match_and_completion_alert.sql
-- Desacopla cadastro da Medicao da Programacao (link opcional por match automatico)
-- e registra snapshot de status CONCLUIDO/PARCIAL da Programacao para alerta posterior.

alter table if exists public.project_measurement_orders
  add column if not exists programming_completion_status_snapshot text null;

alter table if exists public.project_measurement_orders
  add column if not exists programming_completion_status_snapshot_at timestamptz null;
...
alter table if exists public.project_measurement_orders
  ...
  check (
    ...
    or programming_completion_status_snapshot in ('CONCLUIDO', 'PARCIAL')
  );
