-- 425_add_project_latitude_longitude.sql
-- Adiciona latitude/longitude em projetos, em graus decimais (formato aceito pelo Google Maps,
-- ex.: -23.550520 / -46.633308), atualiza a view de labels e a RPC de escrita.
--
-- As colunas aceitam NULL de proposito: os projetos ja cadastrados nao tem coordenada de origem
-- real e um backfill inventaria dado no historico. A obrigatoriedade e aplicada na API
-- (`src/app/api/projects/route.ts`), que exige as duas coordenadas em cadastro, edicao e
-- importacao em massa -- inclusive ao editar projeto legado, que assim vai sendo preenchido.
--
-- Sem indice novo: latitude/longitude nao sao filtro de listagem nesta tela.
...
alter table if exists public.project
  drop constraint if exists project_latitude_range_check;

alter table if exists public.project
  add constraint project_latitude_range_check
  check (latitude is null or (latitude >= -90 and latitude <= 90));

alter table if exists public.project
  drop constraint if exists project_longitude_range_check;

alter table if exists public.project
  add constraint project_longitude_range_check
...
drop function if exists public.save_project_record(
  uuid, uuid, uuid, text, text, uuid, uuid, uuid, date, uuid, numeric, uuid, uuid, uuid, uuid, uuid, text, text, uuid, text, text, boolean, boolean, boolean, jsonb, timestamptz
);
...
create or replace function public.save_project_record(
  ...
  p_is_withdrawn boolean default false,
...
begin
  if p_project_id is null then
    insert into public.project (
      ...
      coalesce(p_is_withdrawn, false),
      coalesce(p_is_third_party, false),
      null,
      null,
      null,
      p_actor_user_id,
  ...
  set
    ...
    is_withdrawn = coalesce(p_is_withdrawn, false),
    is_third_party = coalesce(p_is_third_party, false),
...
-- Padrao das migrations 210/251/309: RPC SECURITY DEFINER e chamada apenas pelo client
-- service_role no servidor; nenhum EXECUTE para public/anon/authenticated.
revoke all on function public.save_project_record(
  uuid, uuid, uuid, text, text, uuid, uuid, uuid, date, uuid, numeric, uuid, uuid, uuid, uuid, uuid, text, text, uuid, text, text, boolean, boolean, boolean, numeric, numeric, jsonb, timestamptz
) from public;
