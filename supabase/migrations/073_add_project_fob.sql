...
-- Adiciona FOB ao cadastro de projetos e republica a view de labels.
...
alter table if exists public.project
  drop constraint if exists project_fob_exact_length_check;

alter table if exists public.project
  add constraint project_fob_exact_length_check
  check (fob is null or char_length(btrim(fob)) = 10);

create index if not exists idx_project_tenant_fob
  on public.project (tenant_id, fob);
