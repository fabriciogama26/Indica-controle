...
-- Adiciona cancelamento persistente da Programacao e ajusta a RPC de salvamento para ignorar registros cancelados.
...
alter table if exists public.project_programming
  drop constraint if exists project_programming_tenant_project_team_date_key;

create unique index if not exists uq_project_programming_active_project_team_date
  on public.project_programming (tenant_id, project_id, team_id, execution_date)
  where is_active = true;

create index if not exists idx_project_programming_tenant_date_team_active
  on public.project_programming (tenant_id, execution_date, team_id, is_active);

create index if not exists idx_project_programming_tenant_project_active
  on public.project_programming (tenant_id, project_id, is_active, updated_at desc);

alter table if exists public.project_programming
  drop constraint if exists project_programming_cancellation_fields_check;

alter table if exists public.project_programming
  add constraint project_programming_cancellation_fields_check
  check (
    (
      is_active = true
...
drop function if exists public.cancel_project_programming(
  uuid,
  uuid,
  uuid,
  text,
...
create or replace function public.cancel_project_programming(
  p_tenant_id uuid,
  p_actor_user_id uuid,
...
begin
  if p_programming_id is null or v_reason is null then
    return jsonb_build_object(
      'success', false,
      'status', 400,
      'reason', 'INVALID_CANCELLATION_PAYLOAD',
  ...
  if not found then
    return jsonb_build_object(
      ...
      'status', 404,
      'reason', 'PROGRAMMING_NOT_FOUND',
      'message', 'Programacao nao encontrada ou ja cancelada.'
    );
  end if;
  ...
  if p_expected_updated_at is not null
    ...
    return jsonb_build_object(
      ...
      'status', 409,
      'reason', 'PROGRAMMING_CONFLICT',
      'message', 'Esta programacao foi alterada por outro usuario. Atualize a grade antes de cancelar.'
    );
  end if;
  ...
  return jsonb_build_object(
    ...
    'message', format('Programacao do projeto %s cancelada com sucesso.', v_current.sob)
  );
end;
$$;

revoke all on function public.cancel_project_programming(
  uuid,
  uuid,
  uuid,
  text,
...
grant execute on function public.cancel_project_programming(
  uuid,
  uuid,
  uuid,
  text,
