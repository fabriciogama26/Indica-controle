...
-- Adia atomicamente todas as programacoes ativas do mesmo Projeto + Data.

drop function if exists public.postpone_project_programming_group(
  uuid,
  uuid,
  uuid,
  date,
...
begin
  if p_programming_id is null or v_reason is null then
    return jsonb_build_object(
      'success', false,
      'status', 400,
      'reason', 'INVALID_POSTPONE_GROUP_PAYLOAD',
      'message', 'Informe a programacao e o motivo do adiamento.'
    );
  end if;
  ...
  loop
    ...
    if coalesce((v_result ->> 'success')::boolean, false) = false then
      raise exception '%', jsonb_build_object(
        ...
        'message', coalesce(v_result ->> 'message', 'Falha ao adiar uma das programacoes do mesmo projeto e data.'),
        'detail', v_result ->> 'detail',
        'programming_id', v_item.id
      )::text;
    end if;

    v_updated_programming_ids := array_append(
      v_updated_programming_ids,
    ...
    'message',
      case
        when p_new_execution_date is null then
          format(
            '%s programacao(oes) do projeto %s em %s adiada(s) com sucesso.',
            v_affected_count,
          ...
          format(
            '%s programacao(oes) do projeto %s em %s adiada(s) com sucesso. Nova data: %s.',
            v_affected_count,
            coalesce(v_source.sob, v_source.project_id::text),
            to_char(v_source.execution_date, 'DD/MM/YYYY'),
            to_char(p_new_execution_date, 'DD/MM/YYYY')
          )
      end
  );
exception
    ...
    exception
      when others then
        v_structured_error := jsonb_build_object(
          'success', false,
          'status', 500,
          'reason', 'POSTPONE_GROUP_FAILED',
          'message', 'Falha ao adiar programacoes do mesmo projeto e data.',
          'detail', sqlerrm
        );
    end;
...
revoke all on function public.postpone_project_programming_group(
  uuid,
  uuid,
  uuid,
  date,
...
grant execute on function public.postpone_project_programming_group(
  uuid,
  uuid,
  uuid,
  date,
...
declare
  v_function_oid oid :=
    'public.postpone_project_programming_group(uuid,uuid,uuid,date,text,timestamp with time zone)'
      ::regprocedure::oid;
begin
  if has_function_privilege('anon', v_function_oid, 'EXECUTE')
    or has_function_privilege('authenticated', v_function_oid, 'EXECUTE') then
    raise exception
      'postpone_project_programming_group nao pode ser executada por anon/authenticated';
  end if;

  if not has_function_privilege('service_role', v_function_oid, 'EXECUTE') then
    raise exception
      'postpone_project_programming_group deve permanecer executavel por service_role';
  end if;
end;
$$;
