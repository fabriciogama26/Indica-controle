-- 307_add_project_is_third_party.sql
-- Adiciona marcador TERCEIROS em projetos, atualiza view de labels e RPC de escrita.

alter table if exists public.project
  add column if not exists is_third_party boolean;
...
alter table if exists public.project
  alter column is_third_party set default false;

alter table if exists public.project
  alter column is_third_party set not null;

create index if not exists idx_project_tenant_is_third_party
  on public.project (tenant_id, is_third_party);
...
drop function if exists public.save_project_record(
  uuid, uuid, uuid, text, text, uuid, uuid, uuid, date, uuid, numeric, uuid, uuid, uuid, uuid, uuid, text, text, uuid, text, text, boolean, boolean, jsonb, timestamptz
);
