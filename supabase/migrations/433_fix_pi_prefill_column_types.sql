-- 433_fix_pi_prefill_column_types.sql
-- Corrige defeito da migration 432, encontrado pelo teste de fumaca:
-- `COALESCE types text and numeric cannot be matched` ao criar uma PI.
--
-- CAUSA
-- ---------------------------------------------------------------------------
-- `contract.telefone_corporativo` e NUMERIC no banco vivo, nao text. A 432
-- assumiu texto e pos a coluna dentro de um `coalesce` junto com o valor do
-- payload, que e text. O Postgres nao concilia os dois tipos e a criacao da PI
-- passou a falhar inteira.
--
-- A premissa errada veio de `contract.number`, que a migration 413 converteu
-- para text explicitamente; `telefone_corporativo` nunca passou por essa
-- conversao e ficou como a base legada a criou.
--
-- CORRECAO
-- ---------------------------------------------------------------------------
-- Cast explicito para `text` em TODO campo de `contract` e do cadastro de
-- contato usado no pre-preenchimento, e nao so no telefone: as outras colunas
-- tambem nunca foram versionadas com tipo garantido, e um cast a mais nao custa
-- nada. Nenhuma regra muda; so o tipo.
--
-- Nao altera a assinatura: `create or replace` sobre a versao da 432.
...
begin
  ...
  update public.permission_intervention set
    ...
    manager_name = case when v_is_insert
      then coalesce(nullif(btrim(coalesce(p_payload ->> 'managerName', '')), ''), v_contract.nome_gestor::text)
      else nullif(btrim(coalesce(p_payload ->> 'managerName', '')), '') end,
    company_name = case when v_is_insert
      then coalesce(nullif(btrim(coalesce(p_payload ->> 'companyName', '')), ''), v_contract.empresa::text)
      else nullif(btrim(coalesce(p_payload ->> 'companyName', '')), '') end,
    contract_number = case when v_is_insert
      then coalesce(nullif(btrim(coalesce(p_payload ->> 'contractNumber', '')), ''), v_contract."number"::text)
      else nullif(btrim(coalesce(p_payload ->> 'contractNumber', '')), '') end,
    manager_phone = case when v_is_insert
      then coalesce(nullif(btrim(coalesce(p_payload ->> 'managerPhone', '')), ''), v_contract.telefone_corporativo::text)
      else nullif(btrim(coalesce(p_payload ->> 'managerPhone', '')), '') end,
    manager_email = case when v_is_insert
      then coalesce(nullif(btrim(coalesce(p_payload ->> 'managerEmail', '')), ''), v_contract.email::text)
      else nullif(btrim(coalesce(p_payload ->> 'managerEmail', '')), '') end,
    utility_contact_name = case when v_is_insert
      then coalesce(nullif(btrim(coalesce(p_payload ->> 'utilityContactName', '')), ''), v_contact.name::text)
      else nullif(btrim(coalesce(p_payload ->> 'utilityContactName', '')), '') end,
    utility_contact_phone = case when v_is_insert
      then coalesce(nullif(btrim(coalesce(p_payload ->> 'utilityContactPhone', '')), ''), v_contact.telefone_corporativo::text)
      else nullif(btrim(coalesce(p_payload ->> 'utilityContactPhone', '')), '') end,
    utility_contact_email = case when v_is_insert
      then coalesce(nullif(btrim(coalesce(p_payload ->> 'utilityContactEmail', '')), ''), v_contact.email::text)
