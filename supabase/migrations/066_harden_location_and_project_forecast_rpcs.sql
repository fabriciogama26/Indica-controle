...
-- Endurece RPCs de Locacao e previstos de Projetos com concorrencia, limites e validacoes condicionais.

drop function if exists public.save_project_location_plan(uuid, uuid, uuid, text, jsonb, jsonb);
drop function if exists public.save_project_location_material(uuid, uuid, uuid, numeric, uuid, uuid, text);
drop function if exists public.save_project_location_activity(uuid, uuid, uuid, numeric, uuid, uuid, text);
drop function if exists public.save_project_activity_forecast(uuid, uuid, uuid, numeric, uuid, uuid, text);
drop function if exists public.save_project_material_forecast(uuid, uuid, uuid, numeric, uuid, uuid, text, text);
...
begin
  ...
  if p_quantity > 100000 then
    return jsonb_build_object(
      ...
      'message', 'A quantidade do material previsto nao pode ultrapassar 100000.'
    );
  end if;
  ...
  select plm.id, plm.material_code, plm.updated_at
  into v_item
  from public.project_location_materials plm
  where plm.tenant_id = p_tenant_id
    and plm.location_plan_id = v_plan_id
  ...
  v_current_updated_at := v_item.updated_at;
  if p_expected_updated_at is not null
    and date_trunc('milliseconds', v_current_updated_at) <> date_trunc('milliseconds', p_expected_updated_at) then
    return jsonb_build_object(
      'success', false,
      'status', 409,
      'reason', 'LOCATION_ITEM_CONFLICT',
      'message', 'Este material da locacao foi alterado por outro usuario. Atualize a lista antes de salvar.'
    );
  end if;
...
revoke all on function public.save_project_location_material(uuid, uuid, uuid, numeric, uuid, uuid, text, timestamptz) from public;
grant execute on function public.save_project_location_material(uuid, uuid, uuid, numeric, uuid, uuid, text, timestamptz) to authenticated;
grant execute on function public.save_project_location_material(uuid, uuid, uuid, numeric, uuid, uuid, text, timestamptz) to service_role;
...
begin
  ...
  if p_quantity > 100000 then
    return jsonb_build_object(
      ...
      'status', 400,
      'reason', 'QUANTITY_LIMIT_EXCEEDED',
      'message', 'A quantidade da atividade prevista nao pode ultrapassar 100000.'
    );
  end if;
  ...
  select pla.id, pla.activity_code, pla.updated_at
  into v_item
  from public.project_location_activities pla
  where pla.tenant_id = p_tenant_id
    and pla.location_plan_id = v_plan_id
  ...
  if p_expected_updated_at is not null
    ...
    return jsonb_build_object(
      ...
      'status', 409,
      'reason', 'LOCATION_ITEM_CONFLICT',
      'message', 'Esta atividade da locacao foi alterada por outro usuario. Atualize a lista antes de salvar.'
    );
  end if;
...
revoke all on function public.save_project_location_activity(uuid, uuid, uuid, numeric, uuid, uuid, text, timestamptz) from public;
grant execute on function public.save_project_location_activity(uuid, uuid, uuid, numeric, uuid, uuid, text, timestamptz) to authenticated;
grant execute on function public.save_project_location_activity(uuid, uuid, uuid, numeric, uuid, uuid, text, timestamptz) to service_role;
...
begin
  ...
  if p_quantity > 100000 then
    return jsonb_build_object(
      ...
      'status', 400,
      'reason', 'QUANTITY_LIMIT_EXCEEDED',
      'message', 'A quantidade da atividade prevista do projeto nao pode ultrapassar 100000.'
    );
  end if;
  ...
  select paf.id, sa.code, paf.updated_at
  into v_item
  from public.project_activity_forecast paf
  join public.service_activities sa
    on sa.id = paf.service_activity_id
  ...
  if p_expected_updated_at is not null
    ...
    return jsonb_build_object(
      'success', false,
      'status', 409,
      'reason', 'PROJECT_FORECAST_CONFLICT',
      'message', 'Esta atividade prevista foi alterada por outro usuario. Atualize a lista antes de salvar.'
    );
  end if;
...
revoke all on function public.save_project_activity_forecast(uuid, uuid, uuid, numeric, uuid, uuid, text, timestamptz) from public;
grant execute on function public.save_project_activity_forecast(uuid, uuid, uuid, numeric, uuid, uuid, text, timestamptz) to authenticated;
grant execute on function public.save_project_activity_forecast(uuid, uuid, uuid, numeric, uuid, uuid, text, timestamptz) to service_role;
...
begin
  ...
  if p_quantity > 100000 then
    return jsonb_build_object(
      ...
      'status', 400,
      'reason', 'QUANTITY_LIMIT_EXCEEDED',
      'message', 'A quantidade do material previsto do projeto nao pode ultrapassar 100000.'
    );
  end if;
  ...
  select pmf.id, m.codigo, pmf.updated_at
  into v_item
  from public.project_material_forecast pmf
  join public.materials m
    on m.id = pmf.material_id
  ...
  if p_expected_updated_at is not null
    ...
    return jsonb_build_object(
      ...
      'status', 409,
      'reason', 'PROJECT_FORECAST_CONFLICT',
      'message', 'Este material previsto foi alterado por outro usuario. Atualize a lista antes de salvar.'
    );
  end if;
...
revoke all on function public.save_project_material_forecast(uuid, uuid, uuid, numeric, uuid, uuid, text, text, timestamptz) from public;
grant execute on function public.save_project_material_forecast(uuid, uuid, uuid, numeric, uuid, uuid, text, text, timestamptz) to authenticated;
grant execute on function public.save_project_material_forecast(uuid, uuid, uuid, numeric, uuid, uuid, text, text, timestamptz) to service_role;
