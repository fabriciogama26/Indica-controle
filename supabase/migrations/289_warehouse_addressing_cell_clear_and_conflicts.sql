...
-- Adiciona limpeza em lote por posicao (coluna+linha) do enderecamento de almoxarifado
-- e detalha os materiais conflitantes ao salvar uma nova configuracao de mapa.

create or replace function public.clear_warehouse_cell_addresses(
  p_tenant_id uuid,
...
declare
  ...
  v_material_ids uuid[] := '{}';
begin
  if v_coluna = '' or p_linha is null or p_linha <= 0 then
    return jsonb_build_object('success', false, 'status', 400, 'reason', 'INVALID_CELL', 'message', 'Coluna e linha da posicao sao obrigatorias.');
  end if;

  perform pg_advisory_xact_lock(hashtext(p_tenant_id::text || '|' || p_map_id::text || '|CELL|' || v_coluna || '|' || p_linha::text));
  ...
  for v_cleared in
    ...
    where tenant_id = p_tenant_id
      ...
      and coluna = v_coluna
      and linha = p_linha
    returning material_id, coluna, linha, andar, posicao
  loop
    v_count := v_count + 1;
    v_material_ids := array_append(v_material_ids, v_cleared.material_id);
    ...
    insert into public.warehouse_address_history (
      ...
      p_tenant_id,
      p_map_id,
      v_cleared.material_id,
      'ADDRESS_CLEAR',
      jsonb_build_object(
        'reason', 'CELL_CLEAR',
        'previous', jsonb_build_object('coluna', v_cleared.coluna, 'linha', v_cleared.linha, 'andar', v_cleared.andar, 'posicao', v_cleared.posicao)
      ),
      p_actor_user_id,
      p_actor_user_id
    );
  ...
  return jsonb_build_object(
    ...
    'material_ids', to_jsonb(v_material_ids),
    'message', case when v_count > 0
      then format('%s material(is) removido(s) da posicao %s%s.', v_count, v_coluna, p_linha)
      else 'Nenhum material enderecado nesta posicao.'
    end
  );
end;
$$;

revoke all on function public.clear_warehouse_cell_addresses(uuid, uuid, uuid, text, integer) from public;
revoke all on function public.clear_warehouse_cell_addresses(uuid, uuid, uuid, text, integer) from anon;
revoke all on function public.clear_warehouse_cell_addresses(uuid, uuid, uuid, text, integer) from authenticated;
grant execute on function public.clear_warehouse_cell_addresses(uuid, uuid, uuid, text, integer) to service_role;
