...
-- Endurece RLS multi-tenant usando auth.uid() + app_users.ativo = true.
...
for select using (public.user_can_access_tenant(imei_whitelist.tenant_id));
...
for select using (public.user_can_access_tenant(login_audit.tenant_id));
...
for select using (public.user_can_access_tenant(app_error_logs.tenant_id));
...
for select using (public.user_can_access_tenant(materials.tenant_id));
...
for select using (public.user_can_access_tenant(inventory_balance.tenant_id));
...
for select using (public.user_can_access_tenant(requisicoes.tenant_id));
...
for select using (public.user_can_access_tenant(requisicao_itens.tenant_id));
...
for select using (public.user_can_access_tenant(stock_movements.tenant_id));
...
for select using (public.user_can_access_tenant(stock_conflicts.tenant_id));
...
for select using (public.user_can_access_tenant(stock_conflict_items.tenant_id));
...
for select using (public.user_can_access_tenant(sync_runs.tenant_id));
...
for select using (public.user_can_access_tenant(sync_run_steps.tenant_id));
...
for select using (public.user_can_access_tenant(sync_run_alerts.tenant_id));
...
for select using (public.user_can_access_tenant(project_material_balance.tenant_id));
...
for select using (public.user_can_access_tenant(job_titles.tenant_id));
...
for select using (public.user_can_access_tenant(people.tenant_id));
...
'Retorna true quando o auth.uid() atual pertence ao tenant informado e o app_users.ativo esta true.';
