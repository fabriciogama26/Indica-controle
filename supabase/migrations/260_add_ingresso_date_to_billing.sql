-- 260_add_ingresso_date_to_billing.sql
-- Adiciona campo obrigatorio Data Ingresso (data de competencia) ao cabecalho do faturamento.
-- Registros existentes recebem a data de criacao como valor retroativo.

alter table public.project_billing_orders
  add column if not exists ingresso_date date not null default now()::date;

-- Backfill: registros existentes usam a data de criacao
update public.project_billing_orders
set ingresso_date = created_at::date
where true;

-- Remove o default permanente — a coluna deve ser informada explicitamente nas novas insercoes
alter table public.project_billing_orders
  alter column ingresso_date drop default;
...
-- Atualiza save_project_billing_order para aceitar e persistir ingresso_date
...
begin
  ...
  -- Verificacao O(n) de atividades duplicadas (migration 259)
  if (
  ...
  loop
    ...
    insert into public.project_billing_order_items (
      ...
      activity_unit, voice_point, quantity, rate, unit_value, activity_active_snapshot,
...
-- Atualiza save_project_billing_order_batch_partial para extrair e repassar ingresso_date de cada linha
