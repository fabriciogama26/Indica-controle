-- 340_fix_save_stage_unassigned_record.sql
-- Corrige `record "v_current" is not assigned yet` ao CRIAR etapa (INSERT).
--
-- Causa raiz (introduzida na 318 e mantida na 324):
--   `v_current` e declarado como `record` puro e so recebe `select * into` no
--   ramo de UPDATE. A trava de projeto concluido, porem, referenciava o campo
--   nas DUAS pontas de um CASE avaliado no ramo de INSERT:
--
--     if not (case when v_is_insert then v_is_pendencia else v_current.is_pendencia end)
--
--   Para PREPARAR essa expressao o PL/pgSQL precisa resolver o TIPO de
--   `v_current.is_pendencia`, e um `record` sem atribuicao nao tem descritor de
--   tupla — entao `instantiate_empty_record_variable` levanta o erro ANTES de
--   qualquer decisao de ramo. O CASE nao protege: a falha e na preparacao, nao
--   na execucao do ramo.
--
--   Isso explica o comportamento INTERMITENTE: a expressao preparada fica em
--   cache na sessao do backend. Numa conexao do pooler cujo primeiro save foi um
--   UPDATE, `v_current` estava atribuido, a expressao preparou e os INSERTs
--   seguintes daquela conexao passavam. Numa conexao cujo primeiro save foi um
--   INSERT, estourava.
--
-- Correcao (duas camadas, nenhuma muda regra de negocio):
--   1) `v_current` passa a ser `public.programming%rowtype`. Com rowtype o
--      descritor de tupla e conhecido em tempo de compilacao e a variavel nasce
--      com todos os campos NULL em vez de "nao atribuida" — a resolucao de tipo
--      nunca mais falha. O teste `v_current.id is null` de etapa nao encontrada
--      continua valendo (linha nao achada = todos os campos NULL).
--   2) A trava de projeto concluido passa a resolver a flag em IF explicito, de
--      modo que o ramo de INSERT nao referencia `v_current` de forma alguma.
--
-- Corpo restante IDENTICO ao da 324 (diff De/Para do UPDATE_STAGE preservado).
...
declare
  ...
  -- 340: era `record` puro. Com %rowtype a variavel nasce com descritor de tupla
  -- conhecido, entao referenciar um campo nunca levanta "not assigned yet".
  ...
  -- 340: flag efetiva da trava de concluido, resolvida em IF explicito.
...
begin
  ...
  if v_is_insert then
    -- No INSERT vale a flag recebida; `v_current` nao e tocado neste ramo.
    ...
    -- No UPDATE vale a flag ja gravada na etapa.
    v_pendencia_exception := coalesce(v_current.is_pendencia, false);
  end if;

  -- Trava de projeto CONCLUIDO com excecao de pendencia (secao 4.2/9).
...
begin
  ...
  loop
    ...
    if has_function_privilege('anon', v_fn, 'execute')
      ...
      raise exception '340: funcao % ainda executavel por anon/authenticated', v_fn;
    end if;
  end loop;
end;
$$;
