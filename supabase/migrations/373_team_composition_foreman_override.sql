...
-- Permite selecionar o encarregado da Composicao de Equipe independente do cadastro da Equipe.
-- O encarregado escolhido passa a ser gravado na composicao (foreman_person_id) e vira integrante obrigatorio.

begin;

alter table if exists public.team_compositions
  add column if not exists foreman_person_id uuid;
...
begin
  if not exists (
    ...
    where tc.table_schema = 'public'
      ...
      and tc.constraint_name = 'team_compositions_foreman_tenant_fk'
  ) then
    alter table public.team_compositions
      add constraint team_compositions_foreman_tenant_fk
...
with candidate as (
  ...
  join public.teams t
   ...
   and t.id = tc.team_id
  left join lateral (
    select fh.foreman_person_id
    from public.team_foreman_history fh
    where fh.tenant_id = tc.tenant_id
      and fh.team_id = tc.team_id
      and fh.valid_from <= tc.composition_date
      and (fh.valid_to is null or fh.valid_to >= tc.composition_date)
    order by fh.valid_from desc, fh.created_at desc
    limit 1
  ) h on true
  where tc.foreman_person_id is null
),
conflicting as (
  select c.tenant_id, c.composition_date, c.person_id
  from candidate c
  ...
  and (
    ...
    or not exists (
      ...
      where x.tenant_id = c.tenant_id
        and x.composition_date = c.composition_date
...
declare
  ...
  v_foreman_is_member boolean := false;
...
begin
  ...
  if p_composition_date is null
    ...
    or p_foreman_person_id is null
  ...
  if v_foreman.id is null then
    return jsonb_build_object(
      ...
      'message', 'Encarregado invalido ou inativo para o tenant atual.'
    );
  end if;

  if not coalesce(upper(coalesce(v_foreman.job_title_name, '')) like '%ENCARREGADO%', false) then
    return jsonb_build_object(
      'success', false,
      'status', 422,
      'reason', 'FOREMAN_INVALID_JOB_TITLE',
  ...
  if v_work_status = 'NOT_WORKING' then
    ...
    if v_member_person_id is distinct from v_foreman.id
    ...
    if p_composition_id is null then
      ...
      set
        ...
        foreman_person_id = v_foreman.id,
  ...
  select
    ...
    coalesce(bool_or(person_id = p_foreman_person_id), false)
    into v_valid_member_count, v_distinct_person_count, v_foreman_is_member
  from input_members;
  ...
  if not coalesce(v_foreman_is_member, false) then
    return jsonb_build_object(
      'success', false,
      'status', 400,
      'reason', 'FOREMAN_NOT_MEMBER',
