...
-- Backfill de Estado Trabalho em branco para Programacoes inativas.
--
-- Escopo:
-- - Somente status operacional ADIADA ou CANCELADA.
...
candidates as (
  select
    ...
    case
      ...
      when d.history_related_status is not null then
      ...
      when d.concluded_before_id is not null and d.anticipated_catalog_code is not null then
        'INATIVA_ETAPA_POSTERIOR_A_CONCLUIDO'
    ...
    case
      ...
      when d.history_related_status is not null then
        public.normalize_programming_work_completion_code(d.history_related_status)
      when d.previous_project_status is not null then
...
updated as (
  ...
  where pp.tenant_id = validated.tenant_id
    ...
    and pp.status in ('ADIADA', 'CANCELADA')
    and nullif(btrim(coalesce(pp.work_completion_status, '')), '') is null
  returning
    validated.tenant_id,
...
select
  ...
  updated.etapa_number,
  'Backfill automatico de Estado Trabalho em branco para programacao inativa.',
  jsonb_build_object(
  ...
  jsonb_build_object(
    'source', 'migration',
    'migration', '257_backfill_inactive_programming_work_completion_status',
    'rule', updated.backfill_rule,
    'scope', 'inactive_blank_programming_work_completion_status'
  ),
  null
from updated;
