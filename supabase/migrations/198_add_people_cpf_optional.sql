...
-- Adiciona CPF opcional ao cadastro de Pessoas.

alter table if exists public.people
  add column if not exists cpf text;

alter table if exists public.people
  drop constraint if exists chk_people_cpf_format;

alter table if exists public.people
  add constraint chk_people_cpf_format
  check (cpf is null or cpf ~ '^[0-9]{11}$');

create index if not exists idx_people_tenant_cpf
...
revoke all on function public.save_person_record(uuid, uuid, uuid, text, text, uuid, uuid, text, text, jsonb, timestamptz) from public;
grant execute on function public.save_person_record(uuid, uuid, uuid, text, text, uuid, uuid, text, text, jsonb, timestamptz) to authenticated;
grant execute on function public.save_person_record(uuid, uuid, uuid, text, text, uuid, uuid, text, text, jsonb, timestamptz) to service_role;
