-- 138_add_trafo_position_page_permissions.sql
-- Inclui a tela de posicao unitaria de TRAFO em app_pages e faz backfill de permissoes.
...
values
  (
    'posicao-trafo',
    '/posicao-trafo',
    'Posicao Unitaria TRAFO',
    'Almoxarifado',
    'Consulta unitaria da posicao atual de cada TRAFO por Serial e LP.'
  )
...
select
  ...
  pages.page_key,
  case
    when roles.role_key = 'viewer' then true
    else true
  end as can_access
from (
...
join public.app_pages as pages
  on pages.ativo = true
 and pages.page_key = 'posicao-trafo'
