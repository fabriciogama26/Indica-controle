...
-- Unifica a Medicao para usar somente service_activities como catalogo oficial.
-- Mantem compatibilidade dos itens historicos migrando os IDs gravados.

-- 1) Remove FK antiga (ainda apontando para measurement_activities) antes do backfill.
...
-- 2) Backfill dos itens da medicao para IDs de service_activities por codigo.
update public.project_measurement_order_items as i
set service_activity_id = sa.id
from public.measurement_activities ma
join public.service_activities sa
  on sa.tenant_id = ma.tenant_id
 and sa.code = ma.code
where ma.tenant_id = i.tenant_id
  and ma.id = i.service_activity_id
  and i.service_activity_id is distinct from sa.id;

-- 3) Fallback: quando o ID atual nao existir em service_activities, tenta pelo snapshot activity_code.
update public.project_measurement_order_items as i
set service_activity_id = sa.id
from public.service_activities sa
where sa.tenant_id = i.tenant_id
  and sa.code = i.activity_code
  and not exists (
    select 1
    from public.service_activities sx
    where sx.id = i.service_activity_id
  )
  and i.service_activity_id is distinct from sa.id;
...
begin
  ...
  where not exists (
    select 1
    from public.service_activities sa
    where sa.id = i.service_activity_id
  );

  if v_unresolved_count > 0 then
    raise exception 'Nao foi possivel mapear % itens da medicao para service_activities. Corrija os codigos e rode novamente.', v_unresolved_count;
  end if;
end;
$$;

-- 4) Reforca FK dos itens para service_activities.
alter table if exists public.project_measurement_order_items
...
-- 5) Save da medicao passa a resolver atividade diretamente em service_activities.
...
-- 6) Remove tabela legada apos unificacao.
drop table if exists public.measurement_activities;
