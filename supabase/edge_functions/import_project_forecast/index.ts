...
// Imports project forecast XLSX (codigo, quantidade) with RPC guard.
...
serve(async (req) => {
  ...
  const missingCodes = codeList.filter((code) => !materialByCode.has(code))
  if (missingCodes.length > 0) {
    return respond(400, {
      success: false,
      message: 'Existem codigos de material nao cadastrados no tenant.',
      errors: missingCodes.slice(0, 50).map((code) => `Material nao encontrado: ${code}`),
    })
  }

  const materialIdsInFile = parsed.rows.map((row) => materialByCode.get(row.code)?.id).filter(Boolean) as string[]
  ...
  const { data: precheckData, error: precheckError } = await supabase.rpc('precheck_project_material_forecast_import', {
    ...
    p_material_ids: materialIdsInFile,
  })
  ...
  if (precheckResult.success !== true) {
    ...
    const codeList = Array.isArray(precheckResult.codes) ? precheckResult.codes.map((v) => String(v)) : []
  ...
  const payload = Array.from(mergedByCode.entries()).map(([code, qty]) => ({
    material_id: materialByCode.get(code)?.id,
    qty_planned: qty,
  }))
  ...
  return respond(200, {
    success: true,
    message: `Materiais previstos do projeto ${String(project.sob ?? '')} importados com sucesso.`,
    summary: {
      projectId: String(project.id),
      projectSob: String(project.sob ?? ''),
      rowsRead: parsed.rows.length,
      materialsRegistered: Number(appendResult.inserted ?? 0),
