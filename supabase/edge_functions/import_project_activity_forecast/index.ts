...
serve(async (req) => {
  ...
  for (const [projectId, projectRows] of rowsByProject.entries()) {
    ...
    const { data: existingRows, error: existingError } = await supabase
      ...
      .in('service_activity_id', activityIdsInProject)

    if (existingError) {
      return respond(500, { success: false, message: 'Falha ao validar atividades ja cadastradas no projeto.' })
    }

    const existingActivityIds = new Set(
      (existingRows ?? []).map((item: { service_activity_id: string }) => String(item.service_activity_id)),
    )
    ...
    const { data: precheckData, error: precheckError } = await supabase.rpc('precheck_project_activity_forecast_import', {
      ...
      p_activity_ids: rowsToInsert.map((row) => activityByCode.get(row.code)?.id).filter(Boolean) as string[],
    })
  ...
  return respond(200, {
    ...
    message:
      skipped > 0
        ...
        : `Atividades previstas importadas com sucesso. Projetos processados: ${processedProjectIds.size}.`,
